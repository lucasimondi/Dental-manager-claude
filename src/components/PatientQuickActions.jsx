...
export default function PatientQuickActions({ patient, setPatients, setPayments, onNewAppointment }) {
  ...
  const updatePatient = (change) => setPatients?.((rows) => rows.map((row) => row.id === patient.id ? { ...row, ...change } : row));
  const saveAnnotation = () => {
    if (!annotation.trim()) return;
    updatePatient({ annotazioni: [...(patient.annotazioni || []), { id: Date.now(), data: today(), testo: annotation.trim(), richiamo: recallDate ? { testo: annotation.trim(), data: recallDate, fatto: false } : null }] });
  ...
  return <Crd style={{ marginTop: 10 }}>
    ...
    {modal === 'note' && <Modal title="Note generali" onClose={() => setModal(null)}><Txt value={note} onChange={(event) => setNote(event.target.value)} /><div style={{ marginTop: 10 }}><Btn ch="Salva" onClick={() => { updatePatient({ noteGenerale: note }); setModal(null); }} full /></div></Modal>}
