...
export default function PoliedronPanel({
  ...
  highlightedIndex, onHighlightChange, onSelectResult, onConfirmAction, onModifyAction,
  onClose, inputRef,
}) {
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === 'ArrowDown' && total > 0) {
      ...
      else if (state?.suggestedActions?.[0]) onConfirmAction(state.suggestedActions[0]);
    }
  };
  ...
  const containerStyle = isMobile
    ...
    : {
        ...
        width: 'min(560px, calc(100vw - 48px))', maxHeight: '70vh', display: 'flex', flexDirection: 'column',
      ...
      >
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: isMobile ? '12px 14px' : '14px 16px' }}>
          {state?.answer != null ? (
            ...
            <div style={{ padding: '24px 8px', textAlign: 'center', fontSize: 12.5, color: C.txl }}>Sto cercando…</div>
          ) : null}
        </div>
