...
export default function Odontogramma({ selected, onChange, onDenteChange }) {
  const toggle = (d) => {
          ...
          return (
            <React.Fragment key={d}>
              ...
              <button
                ...
                title={`Dente ${d}`}
                style={{
                  width: w, height: h, borderRadius: br, background: sel ? C.pri : C.sur,
                  border: `1.5px solid ${sel ? C.pri : C.brd}`, cursor: 'pointer', padding: 0, flexShrink: 0,
                  display: 'flex', flexDirection: 'column', alignItems: 'center',
                  justifyContent: sup ? 'flex-end' : 'flex-start', paddingTop: sup ? 0 : 2, paddingBottom: sup ? 2 : 0,
                  transition: 'background 0.1s,border 0.1s', boxShadow: sel ? `0 0 0 1px ${C.pri}40` : 'none',
                }}
              >
