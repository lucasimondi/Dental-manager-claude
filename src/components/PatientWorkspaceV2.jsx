...
function EconomyDetail({ onAction, model, realMode }) {
  const values = realMode ? [['Preventivato', model.total], ['Pagato', model.paid], ['Residuo', model.outstanding]] : [['Preventivato',4800],['Accettato',4200],['Eseguito',2100],['Pagato',1700],['Residuo',400]];
  ...
  return <div className="pw2-economy-detail"><div className="pw2-economy-grid">{values.map(([label,value]) => <button key={label} className={ECON_TONE[label]}><small>{label}</small><strong>{fmt(value)}</strong></button>)}</div><section><div className="pw2-subhead"><h3>Pagamenti</h3><button onClick={() => onAction('payment')}>Registra pagamento</button></div>{paymentRows.map((row, index) => <div className="pw2-payment-row" key={`${row[0]}-${index}`}><span>{row[0]}</span><strong>{row[1]}</strong><small>{row[2]}</small></div>)}</section><section className="pw2-payment-plan"><div className="pw2-subhead"><h3>Piano pagamenti</h3><button onClick={() => onAction('paymentPlan')}>Nuova rateizzazione</button></div>{realMode ? <p>Nessun modello autorevole disponibile: configurazione solo locale, non persistente.</p> : <><strong>3/5 pagate</strong><span>Prossima: €500 · 15/09/26</span><em>⚠ 1 rata scaduta</em><div className="pw2-installments" data-entity="INSTALLMENT">{[['€500','15/07','PAID'],['€500','15/08','PAID'],['€500','15/09','OVERDUE'],['€500','15/10','PENDING']].map((row) => <div key={row[1]} className={INSTALLMENT_TONE[row[2]]}><b>{row[0]}</b><span>{row[1]}</span><small>{row[2]}</small></div>)}</div></>}</section></div>;
}
...
function QuickCreateDrawer({ kind, plans, context, model, realMode, onClose, onChangeKind }) {
  ...
  return <DetailDrawer title={titles[kind]} onClose={onClose}>
    ...
    {kind === 'economy' && <EconomyDetail onAction={onChangeKind} model={model} realMode={realMode} />}
...
export default function PatientWorkspaceV2({ patient, plans, payments, appointments, studioInfo = {}, documentClient, realMode = false, capabilities = {}, canonicalFinancial, onClose = () => {}, onEdit = () => {} }) {
  ...
  const clinicalFinancialTimeline = useMemo(() => buildClinicalFinancialTimeline({ plans, payments, appointments, documents: documentTimeline }), [plans, payments, appointments, documentTimeline]);
  ...
  return <div className="pw2-shell">
    ...
    <main className="pw2-main">
      ...
      <section className="pw2-economy" data-entity="PAYMENT"><div><span>Situazione economica {canonicalFinancial ? '· snapshot canonico studio disponibile' : ''}</span><h2><i className="pw2-econ-dot pw2-econ-green" aria-hidden="true" />Pagato {fmt(model.paid)}</h2><p><i className="pw2-econ-dot pw2-econ-red" aria-hidden="true" />Residuo {fmt(model.outstanding)}{realMode ? ' · rateizzazione non persistente' : ''}</p></div><button onClick={() => setQuickCreate('economy')}>Dettagli →</button></section>
    ...
    {quickCreate && !['prescription', 'consent'].includes(quickCreate) && <QuickCreateDrawer kind={quickCreate} plans={model.patientPlans} context={workspaceContext} model={model} realMode={realMode} onClose={() => setQuickCreate(null)} onChangeKind={setQuickCreate} />}
    {quickCreate === 'prescription' && <React.Suspense fallback={<div className="pw2-real-modal"><div className="pw2-doc-state">Caricamento Ricetta…</div></div>}><DocMedico paz={patient} si={studioInfo} initialType="ricetta" onClose={() => setQuickCreate(null)} onDocumentSaved={() => { setDocumentsReloadToken((value) => value + 1); setTab('doc'); }} /></React.Suspense>}
