...
function QuickCreateDrawer({ kind, plans, context, onClose, onChangeKind }) {
  ...
  return <DetailDrawer title={titles[kind]} onClose={onClose}>
    ...
    {kind === 'paymentPlan' && <form className="pw2-quick-form" onSubmit={(event) => event.preventDefault()}><div className="pw2-form-grid"><label><span>Importo totale</span><input defaultValue="3000" /></label><label><span>Acconto</span><input defaultValue="500" /></label><label><span>Da rateizzare</span><input value="2500" readOnly /></label><label><span>Numero rate</span><input defaultValue="5" /></label><label><span>Frequenza</span><select><option>Mensile</option><option>Bimestrale</option><option>Personalizzato</option></select></label><label><span>Prima scadenza</span><input type="date" defaultValue="2026-09-15" /></label></div><div className="pw2-installment-preview"><strong>5 rate da €500</strong><span>15/09 · 15/10 · 15/11 · 15/12 · 15/01</span></div><button data-action="CREATE_PAYMENT_PLAN" className="pw2-prototype-submit">Crea anteprima rateizzazione <small>Nessuna persistenza</small></button></form>}
...
export default function PatientWorkspaceV2({ patient, plans, payments, appointments, onClose = () => {}, onEdit = () => {} }) {
  ...
  const visiblePlanRows = clinicalRows.slice(0, 5).filter((item) => planFilter === 'Tutte' || (planFilter === 'Eseguite' && item.status === 'Eseguita') || (planFilter === 'In corso' && item.status === 'In corso') || (planFilter === 'Da fare' && !['Eseguita','In corso'].includes(item.status)));
