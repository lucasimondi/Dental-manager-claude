...
function Timeline({ documentEvents = [] }) {
  const sourceEvents = documentEvents.map((event) => [fmtD(event.occurredAt), event.type === 'PRESCRIPTION_CREATED' ? 'Rx' : '📄', event.label, '']);
  const events = [...sourceEvents, ['25 agosto 2026','✓','Ricostruzione con perno 26','€250'],['20 agosto 2026','💳','Pagamento','€500'],['18 agosto 2026','✓','Endodonzia 26',''],['10 agosto 2026','📄','Preventivo #014 inviato WhatsApp',''],['8 agosto 2026','🦷','Piano clinico creato',''],['5 agosto 2026','📅','Visita','']];
...
export default function PatientWorkspaceV2({ patient, plans, payments, appointments, studioInfo = {}, documentClient, onClose = () => {}, onEdit = () => {} }) {
  ...
  const workspaceContext = useMemo(() => createPatientWorkspaceContext({ patient, activeClinicalPlan: model.activePlans[0], clinicalPlans: model.patientPlans, treatments: clinicalRows, anatomicalContext: clinicalRows.map((item) => ({ type: item.site === 'Generale' ? 'GENERAL' : 'TOOTH', value: item.site })), alerts: [{ type: 'UNSCHEDULED_TREATMENT', severity: 'medium', source: 'TREATMENT', dueAt: '2026-08-29', status: 'OPEN' }], quotes: [{ id: 'quote-014', status: 'SENT', total: 1152 }], payments: model.paidRows, paymentPlans: [{ id: 'pp-1', total: 3000, status: 'ACTIVE' }], installments: [{ amount: 500, due_date: '2026-09-15', status: 'OVERDUE' }], appointments, recalls: [{ type: 'HYGIENE', dueAt: '2027-02-25' }], followups: [], documents, prescriptions: documents.filter((item) => item.category === 'prescriptions'), consents: documents.filter((item) => item.category === 'consents'), automationRules: [{ trigger: 'TREATMENT_COMPLETED(HYGIENE)', condition: 'NO_FUTURE_APPOINTMENT(HYGIENE)', action: 'CREATE_RECALL(+6 MONTHS)' }], timeline: documentTimeline }), [patient, model.activePlans, model.patientPlans, model.paidRows, clinicalRows, appointments, documents, documentTimeline]);
  ...
  return <div className="pw2-shell">
    ...
    {quickCreate && !['prescription', 'consent'].includes(quickCreate) && <QuickCreateDrawer kind={quickCreate} plans={model.patientPlans} context={workspaceContext} onClose={() => setQuickCreate(null)} onChangeKind={setQuickCreate} />}
