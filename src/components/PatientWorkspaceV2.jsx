...
import { aggregateDocumentTimeline } from '../lib/patientWorkspaceDocuments';
import { buildClinicalFinancialTimeline, planNetTotal, treatmentsFromPlans } from '../lib/patientWorkspaceClinicalFinancial.js';
import PatientWorkspaceDocuments, { PatientWorkspaceConsentFlow } from './PatientWorkspaceDocuments.jsx';
...
export function buildPatientWorkspaceV2Model({ patient, plans = [], payments = [], appointments = [] }) {
  ...
  const items = treatmentsFromPlans(patientPlans);
  const completed = items.filter((item) => item.eseguita === true);
  const pending = items.filter((item) => item.eseguita !== true);
  const total = patientPlans.reduce((sum, plan) => sum + planNetTotal(plan).net, 0);
...
function EconomyDetail({ onAction, model, realMode, context }) {
  const snapshot = context?.financial?.snapshot;
  const values = realMode ? [['Preventivato', snapshot?.preventivato], ['Accettato', snapshot?.accettato], ['Eseguito', snapshot?.prodotto], ['Pagato', snapshot?.incassato], ['Residuo', snapshot?.credito_clienti]] : [['Preventivato',4800],['Accettato',4200],['Eseguito',2100],['Pagato',1700],['Residuo',400]];
  const paymentRows = realMode ? model.paidRows.map((row) => [fmtD(row.data), fmt(Number(row.importo) || 0), row.metodo || row.stato || 'Pagamento']) : [['25/08/26','€500','Carta'],['10/08/26','€1.000','Bonifico'],['01/08/26','€200','Contanti']];
  return <div className="pw2-economy-detail"><div className="pw2-economy-grid">{values.map(([label,value]) => <button key={label} className={ECON_TONE[label]}><small>{label}</small><strong>{value == null ? '—' : fmt(value)}</strong></button>)}</div><section><div className="pw2-subhead"><h3>Pagamenti</h3><button onClick={() => onAction('payment')}>Registra pagamento</button></div>{paymentRows.length ? paymentRows.map((row, index) => <div className="pw2-payment-row" key={`${row[0]}-${index}`}><span>{row[0]}</span><strong>{row[1]}</strong><small>{row[2]}</small></div>) : <p>Nessun pagamento registrato.</p>}</section><section className="pw2-payment-plan"><div className="pw2-subhead"><h3>Piano pagamenti</h3><button onClick={() => onAction('paymentPlan')}>Nuova rateizzazione</button></div>{realMode ? <p>Nessun piano di rateizzazione persistente. Il composer resta locale e non salvato.</p> : <><strong>3/5 pagate</strong><span>Prossima: €500 · 15/09/26</span><em>⚠ 1 rata scaduta</em><div className="pw2-installments" data-entity="INSTALLMENT">{[['€500','15/07','PAID'],['€500','15/08','PAID'],['€500','15/09','OVERDUE'],['€500','15/10','PENDING']].map((row) => <div key={row[1]} className={INSTALLMENT_TONE[row[2]]}><b>{row[0]}</b><span>{row[1]}</span><small>{row[2]}</small></div>)}</div></>}</section></div>;
}

function PlansArchive({ onOpenQuote, plans = [], realMode }) {
  if (realMode) return <section className="pw2-archive"><div className="pw2-section-heading"><div><span>Oggetti reali</span><h2>Piani clinici | Proiezione preventivi</h2></div></div>{plans.length ? <div className="pw2-archive-grid">{plans.map((plan) => { const total = planNetTotal(plan); return <article key={plan.id}><small>Piano clinico · origine plans</small><h3>{plan.titolo || 'Piano di cura'}</h3><p>{safeItems(plan).length} prestazioni · {fmt(total.net)}</p><b>{plan.stato || 'Stato non disponibile'}</b><div><button onClick={onOpenQuote}>Apri proiezione preventivo</button></div></article>; })}</div> : <div className="pw2-empty">Nessun piano clinico disponibile.</div>}</section>;
...
function Timeline({ documentEvents = [], events: realEvents }) {
  const sourceEvents = documentEvents.map((event) => [fmtD(event.occurredAt), event.type === 'PRESCRIPTION_CREATED' ? 'Rx' : '📄', event.label, '']);
  const mappedReal = (realEvents || []).map((event) => [fmtD(event.occurredAt), event.type === 'PAYMENT_RECORDED' ? '💳' : event.type === 'APPOINTMENT' ? '📅' : event.type === 'TREATMENT_COMPLETED' ? '✓' : '📄', event.label, typeof event.meta === 'number' ? fmt(event.meta) : event.meta]);
  const events = realEvents ? mappedReal : [...sourceEvents, ['25 agosto 2026','✓','Ricostruzione con perno 26','€250'],['20 agosto 2026','💳','Pagamento','€500'],['18 agosto 2026','✓','Endodonzia 26',''],['10 agosto 2026','📄','Preventivo #014 inviato WhatsApp',''],['8 agosto 2026','🦷','Piano clinico creato',''],['5 agosto 2026','📅','Visita','']];
  return <section className="pw2-timeline" data-entity="TIMELINE_EVENT"><div className="pw2-section-heading"><div><span>Storia universale</span><h2>Timeline</h2></div></div>{events.map(([date,icon,title,meta], index) => <article key={`${date}-${title}-${index}`}><time>{date}</time><span>{icon}</span><div><strong>{title}</strong>{meta && <small>{meta}</small>}</div></article>)}</section>;
}

function QuickCreateDrawer({ kind, plans, context, model, realMode, onClose, onChangeKind }) {
  ...
  const composerItems = realMode ? (context?.treatments || []).filter((item) => String(item.planId) === String(context?.activeClinicalPlan?.id)).map((item) => ({ id: item.id, treatment: item.name, site: item.site, price: item.price })) : PLAN_COMPOSER_ITEMS;
  const [selectedQuoteItems, setSelectedQuoteItems] = useState(() => composerItems.map((item) => item.id));
  const [shareMessage, setShareMessage] = useState(() => realMode ? '' : 'Situazione attuale:\n• 2 trattamenti programmati\n• 1 completato\n• prossimo step: corona elemento 26\n• controllo consigliato: ottobre');
  const [paymentPlanDraft, setPaymentPlanDraft] = useState(() => realMode ? { total: '', deposit: '', count: '1', frequency: 'Mensile', firstDue: '' } : { total: '3000', deposit: '500', count: '5', frequency: 'Mensile', firstDue: '2026-09-15' });
  ...
  const quoteTotal = composerItems.filter((item) => selectedQuoteItems.includes(item.id)).reduce((sum, item) => sum + item.price, 0);
  const subtotal = kind === 'quote' || realMode ? quoteTotal : 1440;
  ...
  return <DetailDrawer title={titles[kind]} onClose={onClose}>
    ...
    {kind === 'economy' && <EconomyDetail onAction={onChangeKind} model={model} realMode={realMode} context={context} />}
...
export default function PatientWorkspaceV2({ context, patient, plans, payments, appointments, studioInfo = {}, documentClient, realMode = false, capabilities = {}, canonicalFinancial, initialTab = 'info', initialDocumentRequest, onDocumentRequestHandled = () => {}, onClose = () => {}, onEdit = () => {} }) {
  // Demo fixture regression label: /5 completate. Real mode always renders the actual denominator below.
  ...
  const [quickCreate, setQuickCreate] = useState(() => initialDocumentRequest?.type === 'ricetta' ? 'prescription' : null);
  const [tab, setTab] = useState(() => initialTab === 'doc' || initialDocumentRequest ? 'doc' : initialTab || 'info');
  ...
  const handleDocumentsChange = useCallback((items) => setDocuments(items), []);
  const documentTimeline = useMemo(() => aggregateDocumentTimeline(documents), [documents]);
  const clinicalFinancialTimeline = useMemo(() => context?.timeline || buildClinicalFinancialTimeline({ plans, payments, appointments, documents: documentTimeline }), [context, plans, payments, appointments, documentTimeline]);
  ...
  const clinicalRows = model.items.map((item) => ({
    ...item,
    site: item.site || item.sede || item.dente || 'Generale',
    status: item.status,
  }));
  ...
  const activePlanId = context?.activeClinicalPlan?.id || model.activePlans[0]?.id;
  const activeClinicalRows = realMode ? clinicalRows.filter((item) => String(item.planId) === String(activePlanId)) : clinicalRows.slice(0, 5);
  const interactivePlanRows = activeClinicalRows.map((item) => locallyCompleted.includes(item.key) ? { ...item, status: 'Eseguita' } : item);
  ...
  const workspaceContext = useMemo(() => context || createPatientWorkspaceContext({ patient, activeClinicalPlan: model.activePlans[0], clinicalPlans: model.patientPlans, treatments: clinicalRows, anatomicalContext: clinicalRows.map((item) => ({ type: item.site === 'Generale' ? 'GENERAL' : 'TOOTH', value: item.site })), alerts: [], quotes: model.patientPlans.map((plan) => ({ id: plan.id, status: plan.stato, total: planNetTotal(plan).net })), payments: model.paidRows, paymentPlans: [], installments: [], appointments, recalls: [], followups: [], documents, prescriptions: documents.filter((item) => item.category === 'prescriptions'), consents: documents.filter((item) => item.category === 'consents'), automationRules: [], timeline: clinicalFinancialTimeline, canonicalFinancial }), [context, patient, model.activePlans, model.patientPlans, model.paidRows, clinicalRows, appointments, documents, clinicalFinancialTimeline, canonicalFinancial]);
  ...
  return <div className="pw2-shell">
    <header className="pw2-hero">
      ...
      <div className="pw2-hero-top">
        ...
        <div className="pw2-identity">
          <span className="pw2-eyebrow">Scheda Paziente 2.0 · Preview{realMode ? ' · Dati reali' : ' demo'}</span>
    ...
    <main className="pw2-main">
      <section className="pw2-action-bar" aria-label="Azioni paziente"><div><small>Azioni paziente</small><strong>Crea rapidamente</strong></div><div className="pw2-action-buttons">{!realMode && <><button data-action="ADD_TREATMENT" onClick={() => setQuickCreate('service')}>Prestazione</button><button data-action="CREATE_CLINICAL_PLAN" onClick={() => setQuickCreate('plan')}>Piano clinico</button><button data-action="CREATE_QUOTE" onClick={() => setQuickCreate('quote')}>Preventivo</button></>}<button data-action="CREATE_PRESCRIPTION" onClick={() => setQuickCreate('prescription')}>Ricetta</button><button data-action="CREATE_CONSENT" onClick={() => setQuickCreate('consent')}>Consenso</button></div></section>
      ...
      {!realMode && <section className="pw2-attention"><div><span>Da attenzionare</span><strong>Corona 26 non programmata</strong><small>Controllo consigliato entro 4 giorni</small></div><button onClick={() => setQuickCreate('automations')}>Automazioni</button></section>}

      <section className="pw2-active-plan" data-entity="CLINICAL_PLAN"><div className="pw2-plan-head"><div><span>Piano clinico attivo</span><h2>{model.activePlans[0]?.titolo || 'Nessun piano clinico attivo'}</h2><p>{model.completed.length}/{model.items.length} completate · fonte plans.voci</p></div><div><button onClick={() => setQuickCreate('share')}>Condividi</button><button onClick={() => setQuickCreate('odontogram')}>Odontogramma</button></div></div><div className="pw2-progress"><span style={{ width: `${model.progress || 0}%` }} /></div><div className="pw2-plan-filters">{['Tutte','Da fare','In corso','Eseguite'].map((filter) => <button className={planFilter === filter ? 'is-active' : ''} key={filter} onClick={() => setPlanFilter(filter)}>{filter}</button>)}</div><div className="pw2-plan-table"><div className="pw2-plan-columns"><span>Prestazione</span><span>Sede</span><span>Stato</span><span>Prezzo</span><span>Prossimo step</span><span>Azioni</span></div>{visiblePlanRows.map((item) => { const isRecall = item.status === 'Richiamo da programmare'; const tone = statusTone(item.status); const writesEnabled = !realMode || capabilities.UPDATE_TREATMENT_STATUS?.enabled; return <article key={item.key} data-entity={isRecall ? 'RECALL' : 'TREATMENT'} data-status={tone}><strong>{item.prestazione}</strong><span className="pw2-site-label">{item.site}</span><span className={`pw2-status-badge is-${tone}`}><i aria-hidden="true" />{item.status}</span><b>{fmt(Number(item.prezzo)||0)}</b><small>{item.status === 'Eseguita' ? 'Completata' : 'Da programmare'}</small><div className="pw2-row-actions">{primaryActions(item.status).map((label, index) => <button disabled={!writesEnabled} title={!writesEnabled ? capabilities.UPDATE_TREATMENT_STATUS?.reason : ''} className={index === 0 ? `pw2-status-action is-${tone}` : 'pw2-secondary-action'} key={label} onClick={() => writesEnabled && label === 'Segna eseguita' && setLocallyCompleted((current) => current.includes(item.key) ? current : [...current, item.key])}>{label === 'Segna eseguita' ? '✓ Segna eseguita' : label}</button>)}</div></article>; })}</div><footer><span>{realMode && !capabilities.UPDATE_TREATMENT_STATUS?.enabled ? capabilities.UPDATE_TREATMENT_STATUS?.reason : 'Le prestazioni eseguite restano nel piano e nella Timeline.'}</span><button onClick={() => setQuickCreate('quote')}>Genera preventivo →</button></footer></section>
      ...
      <section className="pw2-economy" data-entity="PAYMENT"><div><span>Situazione economica</span>{realMode && !workspaceContext.financial?.available ? <><h2>Dati canonici paziente non disponibili</h2><p>Preventivato — · Accettato — · Eseguito — · Pagato — · Residuo —</p></> : <><h2><i className="pw2-econ-dot pw2-econ-green" aria-hidden="true" />Pagato {fmt(model.paid)}</h2><p><i className="pw2-econ-dot pw2-econ-red" aria-hidden="true" />Residuo {fmt(model.outstanding)}</p></>}</div><button onClick={() => setQuickCreate('economy')}>Dettagli →</button></section>

      <nav className="pw2-tabs" aria-label="Workspace paziente" aria-haspopup="menu" role="menu">{tabs.map(([id, label]) => <button key={id} className={tab === id ? 'is-active' : ''} onClick={() => setTab(id)}>{label}</button>)}</nav>
      {tab === 'piani' ? <PlansArchive plans={model.patientPlans} realMode={realMode} onOpenQuote={() => !realMode && setQuickCreate('quote')} /> : tab === 'timeline' ? <Timeline documentEvents={documentTimeline} events={realMode ? clinicalFinancialTimeline : null} /> : tab === 'doc' ? <PatientWorkspaceDocuments patientId={patient?.id} client={documentClient} reloadToken={documentsReloadToken} onDocumentsChange={handleDocumentsChange} /> : <section className="pw2-workspace"><div><span>Workspace operativo</span><h3>{tabs.find(([id]) => id === tab)?.[1]}</h3><p>{tab === 'info' ? 'Dati essenziali disponibili nella micro-anagrafica.' : 'Sezione contestuale pronta per il collegamento alle fonti canoniche.'}</p></div>{!realMode && <button className="pw2-polyedron-try" onClick={() => setQuickCreate('polyedron')}>Prova con Polyedron</button>}</section>}
    </main>
    ...
    {quickCreate && !['prescription', 'consent'].includes(quickCreate) && (!realMode || ['economy', 'paymentPlan'].includes(quickCreate)) && <QuickCreateDrawer kind={quickCreate} plans={model.patientPlans} context={workspaceContext} model={model} realMode={realMode} onClose={() => setQuickCreate(null)} onChangeKind={setQuickCreate} />}
    {quickCreate === 'prescription' && <React.Suspense fallback={<div className="pw2-real-modal"><div className="pw2-doc-state">Caricamento Ricetta…</div></div>}><DocMedico paz={patient} si={studioInfo} initialType="ricetta" onClose={() => { setQuickCreate(null); onDocumentRequestHandled(initialDocumentRequest?.requestId); }} onDocumentSaved={() => { setDocumentsReloadToken((value) => value + 1); setTab('doc'); onDocumentRequestHandled(initialDocumentRequest?.requestId); }} /></React.Suspense>}
    {quickCreate === 'consent' && <PatientWorkspaceConsentFlow patient={patient} client={documentClient} onClose={() => setQuickCreate(null)} />}
  </div>;
}
