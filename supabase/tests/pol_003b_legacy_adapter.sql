...
(3105,'31000000-0000-4000-8000-000000000001',31005,'2025-01-06','[{"prestazione":"EUR A","prezzo":100,"eseguita":true,"dataEsec":"2025-01-20"},{"prestazione":"EUR B","prezzo":200,"eseguita":true,"dataEsec":"2025-01-21"}]','attivo',30,'eur'),
(3106,'31000000-0000-4000-8000-000000000001',31006,'2025-01-07','[{"prestazione":"Unknown discount","prezzo":75,"eseguita":false}]','attivo',10,'unknown'),
...
BEGIN
  ...
  IF (r.contracts_inserted,r.lines_inserted,r.produced_inserted,r.payments_inserted) IS DISTINCT FROM (4::bigint,6::bigint,3::bigint,1::bigint) THEN RAISE EXCEPTION 'FAIL safe inserts: %',row_to_json(r); END IF;
  IF (r.plans_skipped,r.lines_skipped,r.produced_skipped,r.payments_skipped) IS DISTINCT FROM (2::bigint,1::bigint,1::bigint,2::bigint) THEN RAISE EXCEPTION 'FAIL skip counters: %',row_to_json(r); END IF;
  ...
  SELECT sum(v.net_amount) INTO amount FROM private.financial_line_values_v1 v WHERE v.studio_id='31000000-0000-4000-8000-000000000001'; IF amount IS DISTINCT FROM 670::numeric THEN RAISE EXCEPTION 'FAIL discounts: %',amount; END IF;
  SELECT sum(v.allocated_discount) INTO amount FROM private.financial_line_values_v1 v WHERE v.studio_id='31000000-0000-4000-8000-000000000001' AND v.source_id='3105'; IF amount IS DISTINCT FROM 30::numeric THEN RAISE EXCEPTION 'FAIL eur fixed allocation total: %',amount; END IF;
  SELECT v.net_amount INTO amount FROM private.financial_line_values_v1 v WHERE v.studio_id='31000000-0000-4000-8000-000000000001' AND v.source_id='3105' AND v.source_line_id='1'; IF amount IS DISTINCT FROM 90::numeric THEN RAISE EXCEPTION 'FAIL eur proportional line A: %',amount; END IF;
  SELECT v.net_amount INTO amount FROM private.financial_line_values_v1 v WHERE v.studio_id='31000000-0000-4000-8000-000000000001' AND v.source_id='3105' AND v.source_line_id='2'; IF amount IS DISTINCT FROM 180::numeric THEN RAISE EXCEPTION 'FAIL eur proportional line B: %',amount; END IF;
  SELECT sum(v.net_amount*e.fraction*e.direction) INTO amount FROM public.financial_line_events_v1 e JOIN private.financial_line_values_v1 v ON v.contract_line_id=e.contract_line_id AND v.studio_id=e.studio_id WHERE e.studio_id='31000000-0000-4000-8000-000000000001' AND e.stage='PRODOTTO'; IF amount IS DISTINCT FROM 360::numeric THEN RAISE EXCEPTION 'FAIL produced discount: %',amount; END IF;
  ...
  SELECT count(*) INTO n FROM public.financial_contracts_v1 WHERE studio_id='31000000-0000-4000-8000-000000000001' AND source_table='plans'; IF n<>4 THEN RAISE EXCEPTION 'FAIL tenant A changed'; END IF;
END $test$;
ROLLBACK;
