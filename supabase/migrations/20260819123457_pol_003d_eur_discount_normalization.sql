-- POL-003D — narrow normalization for the verified legacy fixed-euro encoding.
-- This migration replaces the restricted adapter definition only. It never runs it.
-- Production execution remains subject to a separate Product Owner gate.
BEGIN;
...
BEGIN
  ...
  -- Only the verified legacy value "eur" is added as an explicit fixed discount.
  EXECUTE $sql$
...
COMMENT ON FUNCTION private.run_pol_003b_legacy_adapter_v1(uuid) IS
  'Restricted idempotent adapter. Explicitly normalizes verified eur discounts to FIXED; installation never executes it.';
