...
Documenti correlati: `../../AGENTS.md`, `../POLIEDRA_MASTER_CONTEXT.md` (§32 AI Operating
System), `../mission/POLIEDRA_MISSION.md`, `POL-AI-004-proactive-intelligence.md`,
...
esistente (lo stesso toggle/controller già usato da Orb, Edge Dock e Mobile Dock) e passare
...
La persistenza della conversazione, quando verrà implementata, deve essere tenant-safe,
permission-aware e legata all'utente e allo `studio_id`, e deve riusare la memoria/contesto
di Poliedron invece di introdurne una seconda.
...
Rilevato dal repository al commit `b65cdba` (`master`, 24 agosto 2026). Etichette secondo
`AGENTS.md`.

- `VERIFIED` — esiste una sola implementazione Poliedron: `src/components/poliedron/`
  (Orb, Panel, Edge Dock, Mobile Dock, Conversation, Bell, action preview, risultati) e
  `src/lib/poliedron/` (`intentEngine`, `searchEngine`, `contextEngine`,
  `actionRegistry`, `permissionEngine`, `modelGateway`, `poliedraCore`, planner,
  intelligence).
- `VERIFIED` — `src/lib/poliedron/modelGateway.js` è l'unico punto di chiamata al modello;
  il suo header documenta esplicitamente il vincolo.
- `VERIFIED` — `src/components/AssistenteAI.jsx` è la superficie AI preesistente: non è
  più montata nella shell applicativa (vedi i commenti in `src/App.jsx`), è conservata come
  logica interna in attesa di convergenza, e non è un secondo agente attivo.
  Regola: non rimontarla come chat parallela. Ogni sua capacità residua va assorbita in
  Poliedron.
- `VERIFIED` — `src/components/AgenteAISetup.jsx` è la pagina di setup dell'integrazione,
  non un agente.
- `INFERRED` — il campanello e la voce Chat introdotti in POL-UI-015 (PR #51, draft) sono
  placeholder UI che invocano lo stesso `onToggle` di Orb/Edge Dock, senza notification
  engine e senza secondo agente. Base: descrizione della PR e file elencati; PR non
  mergiata, non verificata in esecuzione da questo audit.
- `NOT VERIFIABLE` in questo audit — persistenza conversazione, stati attività e flusso di
  notifica lato database: richiedono accesso allo schema Supabase di produzione, non
  disponibile qui. Il modello di dati per Chat e attività va progettato in un task
  dedicato, con migration versionata e RLS, senza inventare tabelle o campi.
...
9. Le affermazioni nel handoff sono etichettate `VERIFIED`/`INFERRED`/`NOT VERIFIABLE`?
