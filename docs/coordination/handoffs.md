...
- UNRESOLVED / RISKS: authenticated real-phone QA was NOT VERIFIABLE in this sandbox (no browser session, no credentials used) — the fix was verified by tracing the exact mobile-vs-desktop `window.open`-after-`await` behavior difference and by matching the already-proven-working `ArchivioDocs.jsx`/`PannelloInvioDocumento.jsx` pattern, not by a live device reproduction here.
- PREVIEW ACCESS NOTE: the branch preview deployment required Vercel Authentication (project-wide SSO protection on all non-custom-domain deployments, `deploymentType: all_except_custom_domains`) — Product Owner initially couldn't load the preview on mobile because of this, unrelated to the code fix. Resolved by logging into vercel.com in the mobile browser first; protection was intentionally left enabled (no bypass/disable applied) per Product Owner's explicit choice when asked.
- PRODUCT OWNER CONFIRMATION: verified on a real phone that Apri/Stampa in the patient record's Documenti tab now displays an archived document/ricetta. Merged as PR #71, merge commit `070b28f`, on Product Owner's explicit "funziona mergia" instruction. Status: `MERGED`.
