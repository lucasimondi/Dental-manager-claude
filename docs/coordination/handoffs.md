...
# POL-FIN-003 — management control canonical alignment (local implementation)

- Branch `feature/POL-FIN-003-management-canonical`, based on `origin/master@2812d1c`.
- Cockpit, shared management-control financial KPIs and monthly real Projection values now use the single POL-003 `get_financial_snapshot_v1` boundary. No legacy financial RPC fallback remains in these consumers.
- A presentation adapter copies canonical values only. Metrics not returned by POL-003 (`costi_totali`, `ebitda_pct`) remain explicitly unavailable; no browser formula reconstructs them.
- Break-even display/health uses `break_even_raggiunto`; Proiezioni distinguishes the saved cash scenario from real canonical EBITDA and removes their invalid direct delta. The legacy what-if financial output is disabled until a canonical production-based model exists.
- No migration, database/RLS/data write, production deployment, dependency, or provider change.
- Validation at handoff: canonical tests 6/6; full suite 593/593; build passed with unchanged warnings; `git diff --check` clean.
- User-owned untracked screenshots (`current-cockpit.png`, `mobile-review.png`, `premium-cockpit.png`) were preserved and must not be committed.
- Next: final full suite, commit/push, PR and configured preview; no merge without Product Owner approval.
