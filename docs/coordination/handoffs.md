...
## POL-FIN-002-AUDIT — payments/incassi/da-incassare single-source fix (implemented, not yet pushed/PR'd)

- Direct Product Owner request after PR #75/#76 merged: "Fai audit di funzione pagamenti, incassi, da incassare, dimmi come vengono registrati e dove vanno a finire, dobbiamo collegare tutto ad in un posto quando incassiamo, da dovunque venga l'informazione deve finire in un solo contenitore poi deve aggiornare il controllo nel da incassare, e anche la cifra nel piano del paziente, non per forza deve essere assegnata ad una prestazione, anche al piano stesso, e il dato deve essere uguale dovunque sia necessario."
- **Audit performed first, no code touched until findings were confirmed**: read `paymentService.js`, `supabase.js` (`TABLE_MAP`/`DB`), `App.jsx`'s `makeSyncSetter`, the `actionExecutor.js` write path, `useControlloDati.js`, `incassiService.js`/`incassiMath.js`/`incassiActions.js`/`planPaymentAllocation.js`, `Incassi.jsx`, `Piani.jsx`, `SchedaPaz.jsx`, `richiamiBot.js`, `ControlloCockpit.jsx`, `ControlloGestione.jsx`, `Dashboard.jsx`, `PanoramicaControllo.jsx`.
- **Finding presented to the Product Owner**: payment creation already converges on one real container — `public.payments`, reached from every entry point (Piani's quick payment, SchedaPaz's quick payment, the Incassi "Aggiungi da incassare" contextual payment, the AI action path's `buildPendingPayment`) through the same `setPaymentsSync`/`DB.insert`. But a second, LEGACY system ran in parallel and never touched that table: a manual boolean `incassata` per plan item, toggled by "Incassata"/"Da incassare" buttons in `Piani.jsx` and `SchedaPaz.jsx`. That flag fed `useControlloDati.js`'s `esegDaInc`/`totEsegDaInc` (Dashboard's "Eseguito da incassare" box and `ControlloCockpit.jsx`'s cashflow forecast — plus the orphaned, never-rendered `PanoramicaControllo.jsx`) and `richiamiBot.js`'s "eseguita e non incassata" reminder generator — all disconnected from the real `payments` table and from POL-FIN-002's canonical `get_saldo_piano`/`get_saldi_aperti_studio` RPCs, which `Incassi.jsx` and SchedaPaz's own "Situazione finanziaria" widget already used correctly.
- **Three clarifying questions asked via `AskUserQuestion` before touching multiple live financial screens** (financial-data sensitivity, per AGENTS.md's `PRODUCT_OWNER_DECISION_REQUIRED` principle — this is a real product decision, not something to invent):
  1. What should the manual "Incassata/Da incassare" toggle do? Answer: **"Il pulsante da incassare deve aggiornare il da incassare, incassata deve aggiornare il incassato"** — it must have real effect, not stay cosmetic.
  2. Should `ControlloCockpit.jsx`'s cashflow forecast also switch to the real saldo? Answer: **"Sì, aggiorna anche la proiezione."**
  3. Should `richiamiBot.js` use the real saldo instead of the manual flag? Answer: **"Sì, usa il saldo reale."**
- **Interpretation of answer 1** (a free-text answer, not one of the offered multiple-choice options — reasoned explicitly rather than guessed): since the real "incassato"/"da incassare" totals are computed server-side purely from real `payments` rows, the ONLY way a UI toggle can genuinely "update" those totals is if it creates or removes a real payment. So the button's not-yet-incassata state now opens the SAME quick-payment form the existing "Registra pagamento adesso" post-execution checkbox already used, and its already-incassata state reverses that specific payment (after a confirm dialog) rather than just flipping a flag.
- **What shipped**:
  - `Piani.jsx` / `SchedaPaz.jsx`: `toggleIncassata` replaced by `onIncassataClick(plan, itemIndex)`. Not-incassata → `openQuickPayment(plan, itemIndex)` (unchanged, already-existing modal). `saveQuickPayment` now generates the payment's id up front, creates the real payment as before, AND marks the target voce `{ incassata: true, paymentId }` in the same call — so the button and the post-execution checkbox are now one real mechanism, not two disconnected ones. Already-incassata → confirms ("Annullare l'incasso? Il pagamento registrato per questa prestazione verrà eliminato."), deletes the payment matching `item.paymentId` (only that one — a legacy item with no `paymentId`, i.e. flagged under the old system with no real payment behind it, just clears the flag with nothing to delete), and clears `incassata`/`paymentId`.
  - `useControlloDati.js`: new `saldiAperti` state, fetched via `fetchSaldiApertiStudio(studioId)` (the exact same RPC `Incassi.jsx` already calls), refetched on `[studioId, enabled, plans, payments]`. `esegDaInc`/`totEsegDaInc` are now built by grouping `saldiAperti` rows with `eseguito_non_pagato > 0` per patient, instead of scanning `plans`/`v.eseguita && !v.incassata`. Kept the exact same output shape (`{ paz, voci: [{ prestazione, prezzoScontato }], tot }`) so `Dashboard.jsx`'s and the dead `PanoramicaControllo.jsx`'s existing render code needed zero changes — each `voci` entry is now one open PLAN (title + its `eseguito_non_pagato`) rather than one executed item, since the RPC's granularity is per-plan; this is a deliberate, disclosed shape change, not a bug. Because `ControlloCockpit.jsx` already consumed `totEsegDaInc` from this same hook for its cashflow forecast, answer 2 (Cockpit) is fixed by this one change with no edit to `ControlloCockpit.jsx` itself. Confirmed via `mcp__github`-style grep that `PanoramicaControllo.jsx` is not imported/rendered anywhere in `App.jsx` or `ControlloGestione.jsx` (the live "Panoramica" tab is a separate POL-003 canonical component, `CanonicalBaseOverview`, unrelated to this flag) — so it's dead code today, fixed for consistency at zero extra cost rather than left to silently diverge further.
  - `richiamiBot.js`: `generaRichiamiBot({ ..., saldiAperti = [] })` — new optional param, default keeps every existing caller/test working unchanged. Point 4 ("prestazioni eseguite ma non ancora incassate") now builds a `Set` of plan ids with `eseguito_non_pagato > 0` from `saldiAperti` and checks plan membership instead of `v.incassata`. `App.jsx` fetches `get_saldi_aperti_studio` once into a new `saldiApertiStudio` state (same pattern as the existing per-patient `saldiPiani` effect, refetched on `[studioId, plans, payments]`) and passes it as `saldiAperti` into the `generaRichiamiBot` call — one fetch, reused by the one caller that needs the studio-wide list outside the `useControlloDati` hook.
  - Incidental, in-scope fix: `richiamiBot.js` imported `./utils` with no extension — resolves fine under Vite but is invalid for plain Node ESM (`node --test`), and was never caught because no test imported `richiamiBot.js` directly before this change. Fixed to `./utils.js` (zero behavior change) specifically so `generaRichiamiBot` could be exercised with real function calls in the new test, rather than only asserted against at the source-text level.
- **Known, disclosed behavior change**: any plan item marked "Incassata" under the OLD system without a real payment ever having been registered for it will now correctly show back up as outstanding everywhere (Dashboard, Incassi, richiami) — because the canonical `payments` table, not the old flag, is now the only source of truth. This is exactly the "il dato deve essere uguale dovunque" the Product Owner asked for, but is a visible change to how any pre-existing such data displays; flagged explicitly rather than silently absorbed.
- Safety: no migration, RLS, or schema change — same tables POL-FIN-002 already writes to (`plans`, `payments`), no new RPC. No silent payment deletion: reverting "Incassata" always confirms first and only ever removes the exact payment created by that same flow (tracked via `paymentId`); any payment registered another way is untouched.
- Validation: full `npm test` 601/601 (10 new — `tests/planExecutionUi.test.mjs` extended with real source assertions on `onIncassataClick` in both files; new `tests/richiamiBotSaldoReale.test.mjs`, which calls the real `generaRichiamiBot` function with synthetic `saldiAperti` rows rather than only reading source text; new `tests/useControlloDatiSaldoReale.test.mjs`, source-level per this repo's established convention for React-adjacent hook modules with no render harness). `npm run build` clean (only the pre-existing chunk-size warning). `git diff --check` clean.
- Not verifiable: authenticated runtime/visual QA — same standing constraint as every prior round, no browser session available here.
- Branch: `claude/pol-ui-017-mobile-home-r2-3pizhn`, restarted from `master@2812d1c` (its prior content, POL-UI-017, was already fully merged as PR #74 — confirmed via `git merge-base --is-ancestor` before reusing the branch name, per this session's "merged PR → restart the branch" rule). Committed locally; not yet pushed, no PR opened — per standing instruction, a PR is opened only when the Product Owner explicitly asks.
- Exact next action: push the branch and report to the Product Owner. Do not open a PR or merge without explicit instruction.
