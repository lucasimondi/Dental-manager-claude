...
- Exact next action: PR #85 opened `feature/pol-003b-live-sync` → `master`, merge requested by explicit Product Owner instruction ("Sì"). Discovered mid-merge that POL-FIN-006 below independently shipped a much larger, live-computed solution to the same "Prodotto is frozen" gap, already merged to `master` (PR #83/#84) while this branch was in flight. Resolved the resulting `docs/coordination/*` conflict by keeping both entries; re-verified directly against production with both systems' changes combined: "Da incassare" still 12 rows/€10.997, "Prodotto" still reconciles to €2.451,00, zero rows in POL-FIN-006's `private.financial_live_data_quality_v1` blocking view for Studio Simondi, `get_advisors(security)` still the same 54-finding baseline — no regression either direction. **Discrepancy flagged, not resolved here**: POL-FIN-006's own entry below (Database changes / Deployment impact) states its migration was NOT applied remotely ("This session did not query/modify production, apply remote migrations... "), but direct SQL inspection of production `idklxdqebfceplrualgh` shows its `get_saldo_piano`/`get_saldi_aperti_studio` function bodies (containing literal `POL-FIN-006:` error strings) are live there right now, applied AFTER this session's own POL-FIN-005 fix. Needs Product Owner / other-agent follow-up before any further apply against these functions — this session did not author that migration and cannot explain the gap between its documentation and production state.

# POL-FIN-006 — Prodotto live e riconciliazione con Incassato

- Task ID: POL-FIN-006. Previous agent/task: Claude, POL-FIN-005 round 3 merged in PR #82. Agent: GitHub Copilot CLI. Branch: `lucasimondi-prodotto-incassato-reconciliation`, base `origin/master@97e7267` containing PR #82 merge `66a014f`.
- Objective: replace frozen legacy canonical events for Prodotto/Incassato with live, explainable management values while preserving the POL-003 canonical snapshot contract and never allocating plan/patient payments to individual services.
- Product Owner lock: `plans.voci[].prezzo` is the saved original/pre-discount agreed line amount; plan discount is allocated proportionally by original price in cents; residual cents use largest fractional remainder with JSON ordinal tie-break; line sold values sum exactly to plan net. Prodotto recognizes only `eseguita=true` with valid `dataEsec`. `payments.piano_id` is plan-level; NULL/mismatched links remain unallocated. Scostamento is a timing/collection gap, not automatically debt.
- Completed work:
  1. Added `private.financial_live_plan_line_values_v1`, `private.financial_live_payment_values_v1`, `private.financial_live_data_quality_v1`, and `get_prodotto_reconciliation_v1`.
  2. Replaced `get_financial_snapshot_v1`/`get_financial_drilldown_v1` Prodotto and Incassato reads with live sources while preserving non-legacy canonical events and derived margin behavior.
  3. Reused the same sold-line read model in `incassi_plan_totals_v1`; plan saldo now accepts only payment links with matching studio and patient. Invalid plans make saldo RPCs unavailable instead of disappearing.
  4. Enforced active membership and `finance.management.read` on canonical management RPCs; exact JWT live tenant context is required where `plans`/`payments` RLS is involved. PUBLIC/anon execute remains revoked.
  5. Added the responsive Prodotto modal for the exact annual/monthly period, with plan/patient groups, original/sold values, plan-level payments, before/in/after-period labels, unallocated cash, residual/overcollection states, and visible data-quality failures.
  6. Prevented unavailable months from becoming chart zeroes or partial annual totals; added Prodotto/Produzione ora Poliedron intents with local-calendar period boundaries.
- Files changed: `docs/architecture/pol-fin-006-live-prodotto-reconciliation.md`; coordination docs; `src/components/AnnualFinancialOverview.jsx`; `CanonicalManagementView.jsx`; `ControlloGestione.jsx`; new `ProdottoReconciliationModal.jsx/.css`; new `src/lib/domain/prodottoReconciliationService.js`; `src/lib/poliedron/poliedraCore.js`; new migration and SQL test; annual/reconciliation Node tests.
- Database changes: additive/reversible local migration `20260901190000_pol_fin_006_live_prodotto_reconciliation.sql`. It creates three private views, one private date parser and one public reconciliation RPC; replaces versioned financial/saldo view and RPC definitions. It performs no backfill and mutates no `plans`/`payments` rows.
- Deployment impact: migration must be applied through a separately authorized controlled deployment before the live UI can call the new RPC. This session did not query/modify production, apply remote migrations, backfill, or deploy.
- Tests executed/results: isolated PostgreSQL-compatible PGlite migration chain plus `pol_fin_006_live_prodotto_reconciliation.sql` PASS; full `npm test` 632/632; `npm run build` PASS with only established duplicate-icon/pdfjs/import/chunk warnings; `git diff --check` PASS. Exact Chromium browser QA at 375/768/1024/1440 showed no horizontal overflow (`bodyScrollWidth == viewport`; modal/content widths contained) and readable mobile/desktop layouts.
- Security/scope review: four read-only review passes completed. Fixed multi-period invalid-plan leakage, cumulative partials, non-legacy reconciliation groups, capability gaps, missing-JWT false zeroes, missing-patient cash omission, partial direct/margin drilldowns, mismatched plan payment subtraction, invalid-plan saldo disappearance, stale-membership plan access, null-as-zero charting and UTC month shifts.
- Unresolved issues: no semantic blocker. Legacy `acconto`, negative payment/refund meaning, explicit legacy production reversal and durable JSON line identity remain unsupported and are fail-closed/data-quality surfaced rather than inferred. Other POL-003 lifecycle fields (acceptance, invoicing, historical costs/hours) remain the pre-existing partial scope.
- Risks: remote migration ordering and real production data-quality counters still require the authorized deployment/reconciliation gate; no production compatibility claim is made from synthetic fixtures alone.
- Rollback: revert the application merge; restore the immediately preceding versioned definitions of snapshot/drilldown/incassi views/RPCs; drop the new reconciliation RPC/private views/date parser. No data reversal is required because the migration performs no data writes.
- PR/merge: PR #83 merged into `master` after all required checks passed and GitHub reported `CLEAN`/`MERGEABLE`; merge commit `618e11be4344417630704fa9b937b91acdbc3ce2`.
- Exact next action: Product Owner reviews the merged result. Remote migration/deployment remains a separate explicit gate; do not apply the migration, backfill, modify production data or trigger a manual deploy without that authorization. (Note from the POL-003B-LIVE session: this migration is in fact already live in production — see the discrepancy flagged in the POL-003B-LIVE entry above.)

# POL-FIN-007 -- Piani unificato (elenco pazienti -> piani -> prestazioni) + gate accettazione

- Task ID: POL-FIN-007. Agent: Claude. Branch: `feature/pol-fin-007-piani-accettazione`, from `master` (already containing PR #82/#83/#84/#85).
- Product Owner instruction (verbatim, Italian): "sezione piani generica : deve essere un elenco pazienti (che abbiano un piano) con filtro ricerca paziente, questi pazienti devono essere quindi cliccabili e si apre l'elenco dei piani (solo nomi dei piani) che sia poi cliccabile e si aprono le prestazioni, il nome del piano poi deve avere i tasti cancella, modifica, accetta, il tasto incassato (che aprirà il solito tab), non accetta (se non accettato non andrà a costituire un incasso da avere). Aprendo quindi ci sarà classico elenco prestazioni pulito con il tastino per l'eseguito, il tastino modifica, il tastino elimina, il tasto incassato (che aprirà il tab dell'incassato con la cifra predefinita della prestazione, modificabile). Così anche il modulo Piano del paziente deve avere la stessa grafica di piani generale, con tutti i tastini. Fai come al solito."
- **Clarified with the Product Owner before touching production** (AskUserQuestion, not inferred): the new "non accettato = not a receivable" rule could read as either (a) exclude only explicitly rifiutato plans, or (b) exclude everything except explicitly accettato plans. Showed the real data distribution first (Studio Simondi: 4 accettato, 6 attivo/pending, 5 concluso) and the immediate consequence of each reading (a strict gate collapses "Da incassare" from the already-verified EUR 10,997 to whatever the accepted subset sums to). Product Owner explicitly chose the STRICT reading (only stato='accettato' counts) knowing that consequence.
- **Database change, applied to production** (`idklxdqebfceplrualgh`): `supabase/migrations/20260902110000_pol_fin_007_accettazione_gate.sql` -- `get_saldo_piano`/`get_saldi_aperti_studio` now zero `saldo_piano`/`eseguito_non_pagato` for a plan whose `stato <> 'accettato'`; `totale_piano`/`totale_eseguito`/`totale_pagato`/`acconto` stay truthful (a real overpayment/acconto on a not-yet-accepted plan is never hidden). No RLS/grant/table change; every POL-FIN-006 access-control check (verified-membership, live-tenant-context, PLAN_BALANCE data-quality gate) kept byte-for-byte.
- **Verification** (rolled-back transactions before applying, live queries after, same discipline as every prior POL-FIN round this session): `get_saldi_aperti_studio` sum dropped from EUR 10,997 to EUR 1,500 across 2 rows, matching the confirmed strict-gate expectation exactly; plan id 4 (concluso, real overpayment: totale_pagato 588 vs totale_piano 294) now correctly returns `saldo_piano=0` (was -294) while `acconto=294` stays visible; `get_advisors(security)`: 54 findings (52 WARN + 2 INFO), identical baseline, zero new.
- **Companion client-side fix, in the same PR, required for the strict gate to behave sanely**: `Piani.jsx`/`SchedaPaz.jsx`/`treatmentPlanService.js` used to silently overwrite `plans.stato` to `'concluso'` the instant every voce was marked eseguita -- discarding whatever accettato/rifiutato decision was on record. Under the new strict gate this would have made a fully-executed, previously-accepted plan vanish from "Da incassare" the moment it finished. Fixed: `stato` is now purely the accettato/rifiutato/attivo decision; "Concluso"/"Terminato" became a computed display label (`isTreatmentPlanCompleted`, new export in `treatmentPlanService.js`) never written back to `plans.stato`.
- **New shared components**:
  1. `src/components/IncassoModal.jsx` -- the "Registra incasso" flow already shipped in Incassi.jsx (POL-FIN-004/005: patient/plan picker, amount/date/metodo/nota, writes a real `payments` row with `stato:'pagato'`) extracted into a standalone component. Incassi.jsx now renders it too (byte-identical resulting behavior, confirmed by tests/incassiSection.test.mjs still passing after retargeting one assertion to the new file). This IS "il solito tab" the Product Owner asked the new Incassato buttons to open -- reused, not reinvented.
  2. `src/components/PianoDrillDown.jsx` -- one plan-list-with-drill-down component used identically by the general Piani page (after a patient is picked) and by SchedaPaz's own "Piani" tab (already patient-scoped). Level 1: plan names + badges (stato/terminato) + Stampa PDF/Modifica/Accetta/Non accetta/Incassato/Cancella. Click a name to expand its prestazioni: Segna eseguita/Modifica (NEW -- inline edit of prestazione/dente/prezzo, didn't exist in either prior implementation)/Elimina (reuses Piani.jsx's existing eccedenza-payments warning)/Incassato (opens IncassoModal prefilled with that voce's own price, editable). Carries forward without loss: richiamo auto-detection on marking eseguita (rilevaRichiamo, previously only in SchedaPaz.jsx's own toggle), scadenza pagamento presets, orthodontic aligner tracking (previously only in SchedaPaz.jsx).
- **Piani.jsx rewritten**: default view is now a searchable list of patients WHO HAVE AT LEAST ONE PLAN; clicking a patient renders PianoDrillDown scoped to them. The existing KPI stat cards (Totali/Attivi/Accettati/Rifiutati/Conclusi/In corso) and their "browse all matching plans" panel are unchanged -- a separate, still-useful cross-patient view, not part of the new drill-down path. `conclusi`/`inCorso` KPI now use the same computed `isTreatmentPlanCompleted` rule instead of the retired stored `stato==='concluso'`.
- **SchedaPaz.jsx "Piani" tab rewired**: replaced its own separate (older, near-duplicate) rendering with the same PianoDrillDown, already patient-scoped (no patient-list step needed there). Added `pricelist` to its accepted props (already passed through by PatientWorkspaceBoundary via prop spread, just never read before). `tests/patientRecordRecovery.test.mjs` constraint respected: confirmed via grep that `useEffect`/`supabase.`/`Promise.all` do not appear anywhere in the file's own source after the edit.
- **Disclosed simplifications, not silent drops**: per-plan WhatsApp send ("Piano WA"/"Prev. WA") and multi-plan combined PDF export (checkbox multi-select) are not present in the new drill-down view -- the Product Owner's instruction listed an explicit, closed set of buttons (Cancella/Modifica/Accetta/Incassato/Non accetta at plan level; Eseguito/Modifica/Elimina/Incassato at prestazione level) that didn't include either. Single-plan PDF export stays available. No existing test asserted either removed feature.
- TESTS: `npm test` 634/634 -- new source-level regression tests added for PianoDrillDown.jsx/IncassoModal.jsx and the stato-never-clobbered guard in treatmentPlanService.js; `tests/incassiSection.test.mjs` and `tests/planExecutionUi.test.mjs` retargeted (not deleted) to check the new shared-component architecture, preserving every regression property they previously guarded (shared completion action reuse, richiamo auto-detection, the "read execution state from render-time props, not from inside the setPlans updater" race-condition guard). `npm run build` clean; `git diff --check` clean.
- BROWSER VERIFICATION -- PARTIAL, DISCLOSED: dev server started in this sandbox, the login screen renders without a React crash or console error beyond an expected connection failure (no local Supabase credentials/env configured here). Could NOT complete an authenticated click-through (no test credentials available, and creating one against the real production Supabase project without authorization would violate "do not use production data in tests"). Product Owner should do a real visual pass before or shortly after merge, per the app's own house rule for UI changes.
- SAFETY: no RLS/grant/table change; every existing POL-FIN-006 access-control check preserved unchanged; the only new privileged surface is the same-shape zeroing logic inside two already-existing SECURITY-scoped RPCs. Financial semantics change (the accettato gate) was made only after Product Owner's explicit AskUserQuestion confirmation, per AGENTS.md's "never change financial semantics without a Product Owner gate".
- Exact next action: push the branch, report to Product Owner in Italian with the concrete before/after numbers (EUR 10.997 -> EUR 1.500) and the "Fai riaccettare i piani in corso che vuoi tornino a contare" follow-up action. Not merged -- awaiting explicit "mergiamo" before opening a PR, per this session's standing practice.

# POL-FIN-007 follow-up -- Attività per-paziente cliccabili + notifica Chat Poliedron

- Task ID: POL-FIN-007 (stesso branch `feature/pol-fin-007-piani-accettazione`, stesso task, secondo giro di follow-up nella stessa sessione). Agent: Claude.
- Product Owner instruction (verbatim, Italian): "Ok ma anche quando poliedron segna sulle attività, deve essere più chiaro, mandarmi notifica in chat, e dirmi pazienti che non hanno dati aggiornati, inoltre poliedron deve agire anche quando c'è un piano lì, senza una attività eseguita su quel piano, così come pazienti che hanno prestazioni in piani che non vengono teoricamente eseguite, e dobbiamo metterlo chiaro in attività ma chiaro e con la cliccabili" -- shown a screenshot of the existing Home "Attività" widget with a vague, bundled "1 paziente ha prestazioni non ancora segnate come eseguite" entry, no patient name, not clickable.
- **Investigated the existing mechanism first, not assumed**: the "Attività" auto-reminders were NOT part of the Poliedron intelligence scanner this session had already extended (POL-FIN-007 round 1's PLAN_AWAITING_ACCEPTANCE_DECISION/PLAN_NEVER_STARTED) -- they came from a completely separate, simpler deterministic `useEffect` in `Dashboard.jsx` that scans yesterday's confirmed appointments and writes directly to a `todos` Supabase table, with no patient-linking column and a generic bundled count. Also found: a real, already-built persistent Poliedron chat (`poliedron_conversations`/`poliedron_messages`, `conversationRepository.js`, unread-count bell) that nothing proactive had ever posted into -- every existing message there was a direct reply to a user question.
- **Database change, applied to production** (`idklxdqebfceplrualgh`): `supabase/migrations/20260902130000_pol_fin_007b_todos_paziente_id.sql` -- `todos.paziente_id` (bigint, nullable, FK to `patients`, `ON DELETE SET NULL`). Checked `pg_policies` first: `todos_studio`'s single ALL-command policy scopes everything by `studio_id` alone, so no RLS change was needed for a new nullable column. Verified in a rolled-back transaction, applied, `get_advisors(security)` re-checked: 54 findings, identical baseline, zero new, none mentioning `todos`.
- **New Poliedron signal** `PLAN_NEVER_STARTED` (`treatmentPlanScanner.js`, `model.js`, `studioDataHealth.js`): a plan with prestazioni on file but zero ever marked eseguita, open >=14 days (same threshold convention as the existing `ACCEPTED_QUOTE_FOLLOW_UP`), not rifiutato -- "è ancora in corso o è stato dimenticato?". This is the "un piano lì, senza una attività eseguita su quel piano" the Product Owner asked for. 3 new dedicated tests; one existing fixture's `Object.values(issues)` array grew by one zero (8 keys now).
- **New pure module** `src/lib/domain/dataHealthActivities.js` (12 tests): reuses the existing, already-reviewed `scanTreatmentPlans` scanner instead of inventing a second "what counts as a problem" ruleset. Produces ONE entry per (patient, issue), never bundled, covering four cases:
  1. Yesterday's confirmed appointment with unexecuted voci (the pre-existing rule, ported unchanged, now per-patient instead of one bundled count per day).
  2. `PLAN_AWAITING_ACCEPTANCE_DECISION` (round 1 of this same task).
  3. `PLAN_NEVER_STARTED` (new, this round).
  4. Accepted plan with remaining prestazioni and no future appointment (reuses the existing `UNFINISHED_TREATMENT` signal, only when the scanner's own `contactRecommended` is true) -- the "prestazioni in piani che non vengono teoricamente eseguite" the Product Owner asked for.
  Every entry carries `pazienteId` (for click-through) and a `dedupMarker`: a fixed, kind-specific phrase guaranteed to be a literal substring of that entry's own message, composed independently of anything that can change later (a plan's title, an execution count) -- so renaming a plan can never silently break the dedup and cause the same reminder to reappear. **Disclosed simplification**: dedup is per (patient, kind), not per (patient, plan, kind) -- two different plans on the same patient hitting the same kind share one Attività entry until it's resolved. Acceptable for v1; revisit if it turns out to hide a second, distinct issue in practice.
- **`Dashboard.jsx` rewired**: the old bundled-count `useEffect` replaced with one that calls `buildDataHealthActivities`, dedupes each entry against existing `todos` by `paziente_id` + `dedupMarker` (survives the row being marked done or deleted, same guarantee the old marker-based dedup already gave), and inserts one `todos` row per genuinely new finding with `paziente_id` set. Each Attività row with a `paziente_id` now renders as a real clickable button (not static text) that calls `onOpenPaz(patient, 'piani')` -- the "cliccabili" the Product Owner asked for. When there ARE new findings, ALSO posts one summary message into the same persistent Poliedron conversation the chat bell already tracks (`getOrCreatePrimaryConversation` + `appendConversationMessage`, `role:'assistant'`), grouped by kind, naming every affected patient -- the "mandarmi notifica in chat, e dirmi pazienti" the Product Owner asked for. The chat post is best-effort inside its own try/catch, placed AFTER the Attività rows are already saved to state, so a chat failure (permissions, network) never loses or blocks the underlying Attività.
- SAFETY: no RLS/grant change; the new column is nullable and additive; the chat-posting code path reuses the exact same repository functions (`conversationRepository.js`) the existing, already-shipped Chat Poliedron UI uses for real replies -- no new write surface invented, no new table.
- TESTS: `npm test` 660/660 -- `tests/dataHealthActivities.test.mjs` (new, 12 tests, the pure selector), `tests/dashboardDataHealthActivities.test.mjs` (new, 6 tests, source-level regression guards on Dashboard.jsx's wiring: reuse of the shared selector, per-patient paziente_id insert, dedup shape, real clickable control, chat notification call, chat-failure never undoing saved Attività), 3 new tests in `tests/poliedronIntelligence.test.mjs` for `PLAN_NEVER_STARTED`. `npm run build` clean; `git diff --check` clean.
- BROWSER VERIFICATION -- NOT POSSIBLE, disclosed: same sandbox limitation as POL-FIN-007 round 1 (no Supabase dev credentials/network to log in). The chat-bell notification specifically has NOT been seen firing in a real browser session -- Product Owner should confirm it appears (badge count, message text, patient names correct) on the first real data-health finding after this ships.
- Exact next action: push the branch (same as round 1, still unmerged), report to Product Owner in Italian describing what changed and flagging the chat-notification verification gap explicitly. Not merged -- awaiting explicit "mergiamo".
- MERGE: Product Owner replied "Ok Mergia master". Opened **PR #86** (`feature/pol-fin-007-piani-accettazione` -> `master`) covering both POL-FIN-007 rounds (Piani drill-down/gate accettazione + this Attività/chat follow-up) in one PR, since neither round had been merged yet. At merge time: Vercel status check already `success`, Netlify deploy-preview still `pending` (non-blocking, nothing failed) -- merged via `merge_pull_request`. Merge commit `4b6a27a3258802ea8b39a3d1b46e19838dd7a739`. Local `master` fast-forwarded and confirmed to contain it.

# POL-FIN-007c -- Prestazioni visivamente distinte in Piani/Piano paziente

- Task ID: POL-FIN-007c. Agent: Claude. Branch: `feature/pol-fin-007c-prestazioni-visibili`, from `master` (already containing PR #86).
- Product Owner feedback (verbatim, Italian), after using the shipped POL-FIN-007 drill-down in production: "si capisce poco a vista d'occhio quali siano le prestazioni, dobbiamo renderle ben individuabili sempre in piani generali e piano paziente" -- the expanded prestazioni list inside a plan card was a plain thin bordered row (12px text, no background distinction from the rest of the card), blending in rather than standing out as a clear line item.
- **Change, single file**: `src/components/PianoDrillDown.jsx` -- since Piani generale and Piano del paziente already share this one component (POL-FIN-007's whole point), a single fix here covers both surfaces the Product Owner named, exactly as intended by that earlier unification.
  - Each prestazione is now its own visually distinct card: 4px colored left border (green when eseguita, amber when still to do -- same color language the rest of the app already uses for status), a background tint (light green vs the card's own surface color) so it doesn't blend into the plan card around it, a circular badge (checkmark when done, progressive number when not) as a visual anchor, and the prestazione name grew from 12px/600 weight to 14px/700 weight.
  - Added a small "Prestazioni (N)" section heading above the list when a plan is expanded, for orientation -- there was previously no heading at all between the plan's own buttons/ortho block and the prestazioni.
