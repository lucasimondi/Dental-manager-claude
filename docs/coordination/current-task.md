...
- VALIDATION: `npm test` 757/757 (`tests/attivitaCategorie.test.mjs` 10 test + `tests/poliedronHub.test.mjs` aggiornato per il nuovo tasto Attività e per l'handler generalizzato); `npm run build` pulito; `git diff --check` pulito. Migration applicata in produzione via MCP Supabase (verificata `information_schema.columns` prima e dopo).
- EXACT NEXT ACTION: in attesa che il Product Owner dica "Mergia" — poi: push del branch (già fatto), apertura PR, merge.
