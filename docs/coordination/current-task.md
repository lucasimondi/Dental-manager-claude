...
- VALIDATION: `npm test` 755/755 (`tests/attivitaCategorie.test.mjs`, 9 test: copertura categorie, wiring insert automatico/manuale, badge Home, migration, routing/NAV, CRUD Attivita.jsx sezione Attività, sezione Promemoria con stesso stato di Richiami.jsx, consolidamento PoliedronHub); `npm run build` pulito (chunk `Attivita` ~12kB); `git diff --check` pulito. Migration applicata in produzione via MCP Supabase (verificata `information_schema.columns` prima e dopo).
