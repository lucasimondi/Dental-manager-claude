...
- EXACT NEXT ACTION: push `feature/pol-003b-live-sync` and report to Product Owner. Not merged yet — no client-code changes are involved, but per standing policy still awaiting an explicit "mergiamo"/PR instruction before opening one. Product Owner may want to watch "Prodotto" over the next few real treatments marked eseguita to confirm it updates live in the actual UI (not just verified at the SQL level here).
