...
- BRANCH: `feature/pagamenti-piano-esplicito`, base `origin/master`.
- STATUS: IMPLEMENTED_LOCALLY_NOT_APPLIED — codice e migration scritti e validati in locale; **la migration NON è stata applicata in produzione** su esplicita richiesta del Product Owner ("Non ancora, fermati qui" in risposta alla domanda di conferma pre-apply).
...
- NON FATTO / BLOCCATO IN ATTESA DI APPROVAZIONE: `apply_migration` in produzione (`idklxdqebfceplrualgh`) — richiede esplicito via libera del Product Owner. Dopo l'apply restano da fare: `get_advisors(security)` (POL-FIN-002 usò questo stesso pattern, zero nuovi finding attesi dato che RLS non cambia); confronto before/after finale su Studio Simondi con la migration realmente applicata (lo snapshot "before" sopra è già catturato); niente PR aperta (non richiesta); niente merge.
...
- EXACT NEXT ACTION: attendere istruzione esplicita del Product Owner per applicare la migration in produzione (`apply_migration`) — poi `get_advisors(security)`, confronto before/after finale, e solo allora proporre il merge (nessuna PR/merge senza approvazione esplicita, come da `AGENTS.md`).
