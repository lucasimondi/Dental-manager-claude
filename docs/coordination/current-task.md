...
- STATUS: implementato, verificato, non ancora mergiato — in attesa di "mergiamo" esplicito, come da prassi di questa sessione.
...
- EXACT NEXT ACTION: push del branch e report al Product Owner. In attesa di "mergiamo" esplicito prima di aprire una PR, come da prassi di questa sessione. Suggerito: il PO riaccetta i piani già in corso/conclusi che vuole tornino a contare come "Da incassare" tramite il nuovo tasto Accetta, dato il gate stretto appena scelto. Suggerito anche: verificare che la Chat Poliedron riceva davvero la notifica riassuntiva (bell/badge) al primo giro utile in produzione, dato che questa parte non è stata (e non poteva essere) verificata in browser autenticato in questa sandbox.
