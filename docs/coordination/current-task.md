...
- TASK: POL-UI-034 — Attività e promemoria: categorie con etichette visibili + sezione Attività dedicata
- TITLE: il Product Owner chiede che le Attività (e i promemoria) siano classificate con etichette visibili (es. "ordinare", "anamnesi mancante", "piano", "dati mancanti" ecc.) e che esista una sezione Attività dedicata, per maggiore chiarezza.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Attività e promemoria: dobbiamo mettere in modo che siano classificate con etichette visibili : tipo ordinare, anamnesi mancante , piano, dati mancanti ecc, e poi deve esserci una sezione attività in modo che sia tutto più chiaro").
...
- **Fix**:
  ...
  5. Nuova pagina `src/components/Attivita.jsx` (mirror di `Richiami.jsx`: StatCard Da fare/Completate, tabbar filtro per categoria, toggle Da fare/Fatte, card con `borderLeft` colorato + badge categoria, click-through al paziente se `paziente_id` risolve, stesso modale "+ Nuova attività" con selettore Categoria) — CRUD diretto su `supabase.from('todos')` (stessa tabella già usata dal widget Home, nessun nuovo stato duplicato). Nuova voce `NAV` (`{ id: 'attivita', l: 'Attività', ic: 'clip' }`, ungated come `richiami`) e route lazy in `App.jsx`; aggiunta anche a `DEF_DOCK_SETTINGS.menuItems` accanto a `richiami`.
- VALIDATION: `npm test` 754/754 (nuovo `tests/attivitaCategorie.test.mjs`, 8 test: copertura categorie, wiring insert automatico/manuale, badge Home, migration, routing/NAV, CRUD Attivita.jsx, consolidamento PoliedronHub); `npm run build` pulito (nuovo chunk `Attivita`); `git diff --check` pulito. Migration applicata in produzione via MCP Supabase (verificata `information_schema.columns` prima e dopo).
