...
- TASK: POL-UI-038 — PWA: aggiornamenti non arrivano su iOS home-screen anche dopo chiusura completa
- TITLE: dopo POL-UI-035 (skipWaiting/clientsClaim), il Product Owner segnala che chiudendo e riaprendo l'app (iPhone, PWA da schermata Home) è ancora tutto come prima — il meccanismo basato su eventi del service worker non arriva mai su questa piattaforma.
- OWNER: CLAUDE, su istruzione diretta del Product Owner. Messaggi verbatim: "L'ho chiusa e riaperta ma è ancora come prima, il tasto impostazioni in home è solo per Dashboard e le attività sono rimaste così come erano" poi, dopo aver confermato via domanda diretta che il dispositivo è "iPhone/iPad, app da schermo Home (PWA installata)": "Non va per favore sistema che ho perso tempo già troppo".
- BRANCH: `fix/pwa-index-html-version-poll-fallback`, da `master` (contiene già PR #101).
- STATUS: IN CORSO — implementato, testato, buildato, verificato empiricamente in un browser reale contro la build vera; non ancora pushato; in attesa di "Mergia" esplicito del Product Owner.

- **Causa**: WebKit/Safari su iOS è noto per non ricontrollare in modo affidabile gli aggiornamenti del service worker per le PWA installate da schermata Home (standalone display), indipendentemente da skipWaiting/clientsClaim essere configurati correttamente (già sistemati in POL-UI-035) — l'intero percorso `onNeedReload` basato su eventi del service worker può semplicemente non scattare mai su questa piattaforma. Prima di questo giro avevo suggerito di eliminare e reinstallare l'icona (soluzione nota per questo genere di blocco), ma il Product Owner chiede una soluzione che non richieda passaggi manuali sul dispositivo.
- **Fix**: un secondo meccanismo di rilevamento aggiornamenti in `App.jsx`, indipendente dal service worker — ri-scarica `index.html` (già servito con `Cache-Control: no-cache, no-store, must-revalidate` da `vercel.json`, più `cache: 'no-store'` lato client) e confronta il nome del file JS principale con hash che Vite stampa lì dentro contro quello effettivamente in esecuzione (`document.scripts`) — un hash diverso vuol dire che è online una build più recente, indipendentemente da qualunque evento di lifecycle del service worker. Parte una volta subito al mount (copre un avvio a freddo dopo che l'app era stata chiusa del tutto — non esiste ancora uno stato "hidden" precedente da cui far scattare un `visibilitychange`), poi di nuovo ogni volta che la pagina torna visibile (`visibilitychange`, copre la ripresa da sospensione — esattamente il caso segnalato come non funzionante da una semplice riapertura) e su un intervallo di sicurezza ogni 10 minuti mentre l'app resta aperta. Stesso banner non distruttivo "tocca per aggiornare" già esistente — mai un reload automatico silenzioso che potrebbe interrompere un form in corso.
- **Verifica empirica**: servita la build reale (`dist/`) via `python3 -m http.server`, caricata in Chromium headless una pagina con un hash "vecchio" finto nel tag `<script>` e fatta eseguire la stessa identica logica contro il vero `index.html` buildato — risultato confermato: `{"currentScriptSrc":".../index-STALEHASH0000.js","latestScriptSrc":".../index-D3Vw2WZd.js","updateDetected":true}`. File di repro e server di test rimossi a fine verifica.
- VALIDATION: `npm test` 774/774 (nuovo `tests/versionPollingFallback.test.mjs`, 4 test — incluso un test che esercita davvero la regex di estrazione, non solo un grep sul sorgente); `npm run build` pulito; `git diff --check` pulito. Nessuna migration.
- EXACT NEXT ACTION: in attesa che il Product Owner dica "Mergia" — poi verificare che il banner "Nuova versione disponibile" compaia davvero sul suo iPhone al prossimo giro, senza bisogno di reinstallare l'icona.
