...
- BRANCH: `fix/pol-fin-009-stale-payment-deadlines`, da `master` (contiene già PR #108).
- STATUS: MERGED — PR #109, merge commit `a6bac82ae57c0a0456684ade30c972602e256982`, su esplicita istruzione del Product Owner ("Mergia").

- **Causa radice**: `scadenzePagamento` in `src/lib/useControlloDati.js` (fonte unica dietro il widget Dashboard, `PanoramicaControllo.jsx` e `ControlloCockpit.jsx`) includeva qualunque piano con una data `scadenzaPagamento` impostata, senza MAI verificare se quel piano fosse già stato saldato — una scadenza restava visibile per sempre, anche dopo che il paziente aveva pagato tutto. Non era una perdita di dati (verificato direttamente sul database: tutti i 79 pazienti e i 24 pagamenti erano intatti) — un bug di visualizzazione pre-esistente (non introdotto dai merge PR #108/#107), mai notato prima perché nessun piano era stato saldato interamente dopo aver impostato una scadenza.
- **Verificato sui dati reali di produzione** (non solo in teoria) prima e dopo il fix: piano di Capraro (405€ di voci, 405€ già collegati via `payments.piano_id` con `stato='pagato'`) e piano di Savalli (224€ di voci, 784€ già collegati) — entrambi completamente saldati, eppure comparivano ancora come scadenza da pagare.
- **Fix**: estratta la logica in una funzione pura ed esportata `computeScadenzePagamento(plans, patients, payments)` in un nuovo `src/lib/domain/scadenzePagamentoService.js` (stessa convenzione di estrazione già usata per `incassiMath.js`/`incassiActions.js`, per poterla testare senza montare l'hook o mockare Supabase) — un piano ora viene escluso quando il suo residuo reale (totale piano meno i pagamenti effettivamente collegati a QUEL piano via `piano_id`, `stato==='pagato'`, la stessa fonte usata da `get_saldo_piano` lato server) è zero. Un pagamento non ancora assegnato a nessun piano (`piano_id` `null`) intenzionalmente NON riduce il residuo di uno specifico piano finché non viene assegnato (coerente con POL-FIN-008/"Pagamenti da assegnare"). `useControlloDati.js` ora importa `calcPlanTot`/`computeScadenzePagamento` da questo nuovo modulo invece di duplicarne la logica inline.
- VALIDATION: `npm test` 815/815 (nuovo `tests/computeScadenzePagamento.test.mjs`, 9 test — scenario esatto segnalato, piano parzialmente pagato, pagamento non assegnato che non offsetta, pagamento su un piano diverso, piano senza scadenza, paziente non trovato, ordinamento, stato non-pagato); verificato via `@babel/parser`/`@babel/traverse` (introdotto in POL-UI-039) che nessun file toccato abbia identificatori irrisolti; `npm run build` pulito. Nessuna migration — solo logica client, nessuna nuova colonna o RPC.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — le scadenze di Capraro e Savalli dovrebbero sparire dal widget non appena il deploy è online.

---

- TASK: POL-UI-042 — Creazione rapida paziente in Agenda (non compilabile + perdita selezione) + tasto "Modifica" in scheda paziente non apriva nulla
- TITLE: il Product Owner segnala tre problemi collegati al flusso paziente: (1) nel popup nuovo appuntamento di Agenda, se il paziente non esiste, il mini-form "crea paziente" non permette di digitare nei campi Nome/Cognome; (2) dopo aver creato il paziente al volo nella stessa sessione, non è più possibile completare la prenotazione — il campo torna all'elenco di ricerca invece di restare sul paziente appena creato; (3) in scheda paziente, il tasto "Modifica" (inteso per correggere l'anagrafica) riportava invece in Agenda senza aprire alcun form.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Quando si apre popup in agenda per prendere appuntamento se non esiste paziente ce la funzione crea che però funziona male, non fa compilare i dati, inoltre poi non è possibile aggiungere appuntamento se hai creato paziente in stessa sessione dal popup ma ti ritorna l'elenco scorrimento per selezionare il paziente in questione e fissargli l'appuntamento, l'esperienza deve essere molto più fluida e semplice, inoltre quando vado in scheda paziente per modificare l'anagrafica clicco modifica ma mi porta in agenda, quando clicchi modifica deve aprirsi il popup per aggiungere e modificare dati. Così poi mergiamo tutto in master compresa attività precedente pushata da te").
- BRANCH: `fix/pol-fin-008-pagato-unassigned-payments`, da `master` (branch riusato: il Product Owner ha esplicitamente chiesto di mergiare insieme anche POL-FIN-008, già pushato su questo stesso branch).
- STATUS: MERGED — PR #108, merge commit `f251e1b5117bd7d8165c2be601b4ce183af98f02`, su esplicita istruzione del Product Owner ("Così poi mergiamo tutto in master compresa attività precedente pushata da te").

- **Causa radice 1 — mini-form "crea paziente" non digitabile**: `SelettorePaziente.jsx` gestiva focus/blur solo sul singolo `<input>` di ricerca (`onFocus`/`onBlur` con un timeout di chiusura di 150ms). `CreaPazienteInline` (il mini-form Nome/Cognome mostrato quando la ricerca non trova nulla) monta i propri campi come elementi DOM SEPARATI dall'input di ricerca — cliccarci dentro per iniziare a scrivere sfoca comunque l'input di ricerca esterno, che dopo 150ms smontava l'intero blocco `CreaPazienteInline` (campi inclusi), interrompendo la digitazione quasi subito dopo averla iniziata.
- **Causa radice 2 — perdita selezione dopo creazione rapida**: `App.jsx`'s `makeSyncSetter` (il wrapper dietro `setPatients`) crea otticamente il paziente con un id temporaneo lato client, poi in background lo salva su Supabase e SOSTITUISCE silenziosamente quell'id con quello reale del server appena la risposta arriva. `creaPazienteRapido` in `Agenda.jsx` restituisce l'id temporaneo, sincrono, a `SelettorePaziente` — che lo mette in `form.pazienteId`. Se lo scambio d'id avviene (come tipico) prima che l'operatore finisca di compilare il resto del form appuntamento, `form.pazienteId` punta ormai a un id che non esiste più in `patients`: `SelettorePaziente` non trova corrispondenza e torna alla modalità di ricerca, esattamente il comportamento segnalato ("ti ritorna l'elenco scorrimento").
- **Causa radice 3 — "Modifica" in scheda paziente**: il bottone "Modifica" di `SchedaPaz.jsx` chiamava una prop esterna `onEdit(paz)` che, nel percorso di render effettivamente attivo (App.jsx non passa un handler funzionante per il rendering standard di `SchedaPaz`, solo per il componente sperimentale `PatientWorkspaceV2` disattivato di default), si limitava a chiudere la scheda e tornare in Agenda — non apriva mai alcun form di modifica.
- **Fix**:
  1. `SelettorePaziente.jsx`: focus/blur spostati dal singolo `<input>` di ricerca al `<div>` contenitore esterno dell'intero widget (gli eventi sintetici React `onFocus`/`onBlur` si comportano come `focusin`/`focusout`, quindi risalgono correttamente da qualunque figlio, campi di `CreaPazienteInline` inclusi). Il nuovo `handleBlur` ignora la chiusura quando `event.relatedTarget` è ancora contenuto nello stesso widget (`event.currentTarget.contains(event.relatedTarget)`), mantenendo il fallback a `setTimeout(150ms)` per i click su elementi non-focusabili (dove `relatedTarget` è `null`).
  2. `Agenda.jsx`: `creaPazienteRapido` ora memorizza `{tempId, nome, cognome}` in un `useRef` (`pendingQuickPatientRef`) appena crea il paziente. Un nuovo `useEffect` osserva `[patients]`: se `form.pazienteId` è ancora l'id temporaneo memorizzato E quell'id non esiste più in `patients` (segno che lo scambio server è avvenuto), cerca in `patients` il paziente con lo stesso nome/cognome e riallinea `form.pazienteId` al suo id reale — nessuna interruzione visibile per l'operatore, che continua a vedere il paziente selezionato durante tutta la compilazione.
  3. Estratto il form "Nuovo/Modifica paziente" (già presente, duplicato solo in `Pazienti.jsx`) in un componente riusabile `src/components/PazienteFormModal.jsx` (`{ patient, si, onSave, onClose }`, dove il chiamante scrive il risultato in `patients`, non il modale stesso). `Pazienti.jsx` refactored per usarlo (stato `editingPatient` invece di `modal`/`form` separati). `SchedaPaz.jsx`: rimossa la prop `onEdit` inutilizzabile, aggiunto stato locale `editOpen`, il tasto "Modifica" ora apre `PazienteFormModal` direttamente nella scheda paziente, salvando tramite le prop `setPatients`/`onPatientChange` già disponibili.
- VALIDATION: `npm test` 806/806 (nuovo `tests/agendaQuickCreatePatientReconciliation.test.mjs`, 3 test sulla riconciliazione id temporaneo→reale; aggiornato `tests/selettorePaziente.test.mjs` per il nuovo focus/blur su contenitore esterno; aggiornato `tests/patientRecordRecovery.test.mjs` per il nuovo wiring `setEditOpen(true)`/`PazienteFormModal` al posto di `onEdit(paz)`); verificato via `@babel/parser`/`@babel/traverse` (introdotto in POL-UI-039) che nessun file toccato (Agenda.jsx, SelettorePaziente.jsx, SchedaPaz.jsx, Pazienti.jsx, PazienteFormModal.jsx) abbia identificatori irrisolti; `npm run build` pulito. Nessuna migration — solo logica/UI client.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-FIN-008 — "Pagato" invisibile per pagamenti registrati prima del piano + tasto assegna mancante per pazienti con un solo piano
- TITLE: il Product Owner segnala che un pagamento di 50€ registrato PRIMA di creare un piano di 100€ per lo stesso paziente non compare mai nello slot "Pagato" (né in alto né altrove) e non ha modo di essere assegnato al piano creato successivamente.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Allora ho registrato un pagamento (fabio di Stefano) 50 euro, nel pagato in alto nello slot pagato non c'è, poi ho creato un piano di 100 euro, quindi mi dice da pagare, deve comparire nel pagato comunque a prescindere, poi nell'elenco dei pagamenti deve esserci il tasto per assegnare pagamento a piano (che in questo caso è stato creato dopo)").
- BRANCH: `fix/pol-fin-008-pagato-unassigned-payments`, da `master` (contiene già PR #107).
- STATUS: MERGED — PR #108, merge commit `f251e1b5117bd7d8165c2be601b4ce183af98f02`, su esplicita istruzione del Product Owner ("Così poi mergiamo tutto in master compresa attività precedente pushata da te").

- **Causa radice 1 — "Pagato" invisibile**: `SchedaPaz.jsx` (sia la barra statistiche in alto sia la card "Situazione finanziaria") mostrava `aggSaldi.totale_pagato` — un valore aggregato dalle righe `get_saldo_piano`, che sono PER-PIANO per definizione (sommano solo i pagamenti già collegati via `payments.piano_id`). Un pagamento registrato PRIMA che esistesse un piano ha `piano_id` sempre `NULL` (nessun piano a cui agganciarlo al momento della scrittura) — anche dopo che un piano viene creato in seguito, quel pagamento resta invisibile a `get_saldo_piano` per sempre, quindi non contava mai nel "Pagato".
- **Causa radice 2 — nessun tasto per assegnare**: `unassignedPaymentsForMultiPlanPatients` (la funzione dietro la lista "Pagamenti da assegnare" di `Incassi.jsx`) includeva un pagamento senza piano SOLO se il paziente aveva PIÙ DI UN piano (`> 1`) — sul presupposto che un paziente con un solo piano avrebbe sempre già ricevuto l'auto-assegnazione (`planAssignmentForPatient`, che assegna in automatico quando esiste esattamente un piano attivo). Presupposto vero SOLO per un pagamento fatto DOPO che il piano esisteva già — falso per un pagamento fatto PRIMA che il paziente avesse alcun piano: quel pagamento resta senza piano per sempre, e con un solo piano (non "più di uno") restava invisibile anche alla lista di assegnazione, esattamente il caso segnalato.
- **Fix**:
  1. Nuovo `totalePagatoPaziente(payments)` in `src/lib/domain/incassiMath.js` — somma pura (nessuna query, nessuna nuova formula lato server) di tutti i pagamenti del paziente con `stato === 'pagato'`, indipendentemente da `piano_id` — stessa identica convenzione già usata dalla KPI "Incassato" di `Incassi.jsx` a livello di intero studio, estesa qui al singolo paziente invece di inventare una seconda definizione di "pagato". `SchedaPaz.jsx` ora usa questo valore sia per la cella "Pagato" della barra statistiche in alto sia per la card "Situazione finanziaria" (testo E barra percentuale "% saldato", per non mostrare due numeri diversi sotto la stessa etichetta). "Da pagare" (`aggSaldi.saldo_piano`, il saldo canonico calcolato server-side) resta INVARIATO — l'unico cambiamento è cosa significa "Pagato" per visualizzazione, non la formula del saldo residuo.
  2. `unassignedPaymentsForMultiPlanPatients` rinominata in `unassignedPaymentsNeedingAssignment` (il vecchio nome non avrebbe più avuto senso) e la condizione `> 1` diventata `> 0`: qualunque pagamento senza piano ora entra nella lista "Pagamenti da assegnare" appena il paziente ha ALMENO un piano (zero piani resta escluso, correttamente, perché non c'è nulla a cui assegnare). L'interfaccia esistente (menu a tendina + tasto "Assegna" già costruiti per il caso multi-piano) funziona identica per il caso a un solo piano — nessun nuovo componente necessario. Aggiornato anche il testo esplicativo in `Incassi.jsx` ("Il paziente ha più piani..." → ora copre entrambi i casi).
- VALIDATION: `npm test` 802/802 (nuovo `tests/incassiMath.test.mjs`, 4 test sul comportamento reale di `totalePagatoPaziente`; nuovo `tests/schedaPazPagatoRealTotal.test.mjs`, 4 test sul wiring in `SchedaPaz.jsx`; aggiornato `tests/incassiActions.test.mjs` per il nuovo nome/comportamento di `unassignedPaymentsNeedingAssignment`, incluso un test dedicato che riproduce esattamente lo scenario segnalato — pagamento prima del piano, poi piano creato dopo; aggiornato un regex ormai obsoleto in `tests/patientQaRecoveryFinal.test.mjs`); verificato anche via `@babel/parser`/`@babel/traverse` (introdotto in POL-UI-039) che nessun file toccato abbia identificatori irrisolti; `npm run build` pulito. Nessuna migration — solo logica/UI client, nessuna nuova colonna o RPC.
- EXACT NEXT ACTION: mergiato insieme a POL-UI-042 (stesso branch/PR #108). Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-UI-041 — Menu Poliedron su mobile: scrollabile fino in fondo anche con tastiera attiva
- TITLE: il Product Owner segnala che il menu Poliedron (icona centrale del dock), su mobile, è scrollabile ma per vedere tutte le azioni veloci bisogna prima chiudere la tastiera — chiede che sia scrollabile fino in fondo anche con la tastiera aperta.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Quando clicco su poliedron icona esce il menu poliedron , e scrollabile ma su mobile per visualizzare tutto bisogna togliere tastiera. Facciamo che sia scrollabile fino a poter selezionare anche con la tastiera attiva tutte le varie azioni veloci , quindi scrollabile fino in fondo").
- BRANCH: `feature/pol-ui-041-poliedron-panel-keyboard-scroll`, da `master` (contiene già PR #106).
- STATUS: MERGED — PR #107, merge commit `bae70b26b7c375d6ffac1c56eff513b1466b6724`, su esplicita istruzione del Product Owner ("Mergia"), dopo verifica su preview live.

- **Causa radice**: `PoliedronPanel.jsx` (il pannello che si apre toccando l'orb Poliedron) su mobile usa `position: fixed; inset: 0` — dimensionato sul VIEWPORT DI LAYOUT completo, che iOS Safari non riduce mai per la tastiera a schermo: la tastiera si limita a sovrapporsi alla parte bassa di quel riquadro fisso, che resta invariato in altezza. L'area scrollabile interna (`flex:1; overflow-y:auto`) ha quindi lo stesso range di scroll reale sia a tastiera aperta sia chiusa — nessuna consapevolezza che una parte del suo spazio è coperta — quindi una volta che la tastiera copre le ultime azioni veloci, nessuno scroll può farle rientrare in vista: chiudere la tastiera (l'accorgimento già trovato dal Product Owner) è l'unico modo per vederle.
- **Fix**: nuovo `src/lib/poliedron/poliedronPanelViewport.js` — `computeMobilePanelViewportRect(visualViewport)`, funzione pura testata direttamente, che legge `window.visualViewport` (già usato altrove in questa stessa cartella, `usePoliedronPosition.js`) — l'altezza/offset REALMENTE visibili, che su iOS Safari si riducono davvero quando la tastiera si apre, a differenza di `env(keyboard-inset-height)` (già usato altrove in questo codebase) che è una feature esclusiva Chromium/VirtualKeyboard-API, sempre `0px` su iOS. `PoliedronPanel.jsx` ora ridimensiona dinamicamente il riquadro fisso mobile (`top`/`height`, invece di `inset:0` fisso) in base a questa misura reale, aggiornata a ogni evento `resize`/`scroll` di `visualViewport` — così la colonna flex (header fisso, contenuto `flex:1 overflow-y:auto`) è sempre calcolata sullo spazio DAVVERO visibile sopra la tastiera, e il suo scroll interno raggiunge naturalmente ogni ultima azione, mai più coperta.
- **Verificato empiricamente** (non solo in teoria): repro reale in Chromium headless con un mock di `visualViewport` che simula l'apertura della tastiera (altezza ridotta di 300px) — prima della simulazione il riquadro resta a piena altezza; dopo, si ridimensiona correttamente e l'ultima azione veloce diventa interamente visibile e raggiungibile via scroll entro l'area realmente visibile, esattamente come richiesto.
- VALIDATION: `npm test` 793/793 (nuovo `tests/poliedronPanelKeyboardScroll.test.mjs`, 4 test — comportamento reale di `computeMobilePanelViewportRect` con/senza tastiera, wiring dei listener `resize`/`scroll` con cleanup corretto, listener attivo solo su mobile); verificato anche che `PoliedronPanel.jsx` non abbia identificatori irrisolti (stesso controllo `@babel/parser`/`@babel/traverse` introdotto in POL-UI-039, per non ripetere quell'errore); `npm run build` pulito. Nessuna migration.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — già verificato su preview live prima del merge.

---

- TASK: POL-UI-039 root cause reale — `ReferenceError: Can't find variable: mobileOverlayHeight`, mai un problema di cache/service worker
- TITLE: dopo tre giri di fix inefficaci (specificità CSS, centraggio, retry-on-chunk-failure, hardReload del service worker), il Product Owner ha mandato uno screenshot della schermata d'errore che mostra il vero messaggio tecnico: un `ReferenceError` reale, non un problema di caricamento/cache.
- OWNER: CLAUDE, dopo aver chiesto esplicitamente al Product Owner il testo esatto sotto il tasto "Ricarica" (nuovo dettaglio aggiunto in POL-UI-040) e la conferma che il problema si presenta identico anche in Safari normale (non solo nella PWA installata) — entrambe risposte decisive per smontare l'ipotesi cache/service-worker e trovare la causa vera.
- BRANCH: `fix/agenda-mobileoverlayheight-scope-bug`, da `master` (contiene già PR #105).
- STATUS: MERGED — PR #106, merge commit `c9559e9fae9240eb98f9d0f79e7ae1efda2a16dd`, su esplicita istruzione del Product Owner ("Mergia").

- **La causa vera, per la prima volta confermata da un messaggio d'errore reale del browser, non da un'ipotesi**: `GridView` (`src/components/Agenda.jsx`, riga 88) è una funzione componente SEPARATA dal componente `Agenda` esportato di default (riga 674) — non la stessa funzione, non chiude sullo stesso scope. Il fix POL-UI-039 (secondo giro, "diamo clearance in alto anche al menu contestuale") aveva aggiunto un riferimento a `mobileOverlayHeight` dentro il blocco JSX del menu contestuale appuntamento (riga 437) — ma quel blocco vive DENTRO `GridView`, mentre `mobileOverlayHeight` è uno `useState` locale di `Agenda` (riga 808). Un vero `ReferenceError`, non un problema di cache: `GridView` semplicemente non ha mai avuto accesso a quella variabile. Scatta solo al tap sull'appuntamento perché il riferimento vive dentro il blocco condizionale `menuApp && (() => {...})()`, mai valutato prima. **Nessuno dei tre giri precedenti (POL-UI-039 CSS, POL-UI-040 retry-chunk, POL-UI-040 hardReload) poteva risolvere questo — non erano la causa, erano tutte ipotesi plausibili ma sbagliate**, scartate solo dopo aver verificato empiricamente ogni deploy su Vercel (READY, chunk Agenda raggiungibile con 200 reale) e aver chiesto direttamente il testo dell'errore, che nessuna delle verifiche CSS/caching precedenti poteva far emergere da sola.
- **Perché nessun controllo precedente l'ha presa**: `npm run build` (esbuild) verifica solo la sintassi, mai se un identificatore si risolve davvero nello scope in cui viene usato; tutti i test di questo file sono verifiche regex sul testo sorgente, mai una vera esecuzione del componente; i repro empirici costruiti per i giri precedenti erano markup HTML statico scritto a mano che imitava le classi CSS reali — mai il vero albero React — quindi non potevano in alcun modo rilevare un bug di scope cross-funzione come questo.
- **Fix**: `mobileOverlayHeight` ora passa correttamente da `Agenda` a `GridView` come prop reale — aggiunto a `gridProps` (l'oggetto già spreadato su entrambe le chiamate `<GridView {...gridProps} .../>`) e aggiunto alla lista di destructuring dei parametri di `GridView`. Nessun'altra modifica necessaria al punto d'uso (riga 437), che ora risolve correttamente.
- **Nuovo strumento di verifica, non solo per questo bug**: nuovo `tests/agendaNoUnresolvedIdentifiers.test.mjs` — usa `@babel/parser`/`@babel/traverse` (già dipendenze transitive della toolchain di build, non una nuova dipendenza) per fare una vera analisi di scope su `Agenda.jsx` e segnalare qualunque identificatore referenziato che non abbia un binding raggiungibile in nessun punto della catena di scope — esattamente la classe di bug che ha rotto la produzione. Il test verifica prima che il checker stesso rilevi davvero un pattern sintetico identico al bug reale (funzione A usa una variabile locale di funzione B), poi lo esegue per davvero su `Agenda.jsx` (zero identificatori irrisolti dopo il fix) — non solo una prova a vuoto.
- VALIDATION: `npm test` 789/789 (nuovo `tests/agendaNoUnresolvedIdentifiers.test.mjs`, 3 test); `npm run build` pulito. Nessuna migration.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — dato che questo è il vero fix, il popup dovrebbe finalmente funzionare non appena il deploy è online (eventualmente dopo un'ultima chiusura/riapertura completa se la sessione era rimasta bloccata da prima).
...
- STATUS: MERGED — PR #105, merge commit `1983fbde4803ccf0b11eb6566c2ecc843e0b4a16`, su esplicita istruzione del Product Owner ("Mergia").
...
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — dato che il fix riguarda proprio il meccanismo di reload/cache, se la sessione è ancora bloccata sulla schermata d'errore di PRIMA di questo deploy potrebbe servire un'ultima chiusura/riapertura completa per riceverlo; da lì in poi il ricaricamento automatico dovrebbe bastare da solo in futuro.
...
- BRANCH: `fix/lazy-chunk-load-retry-error-boundary`, da `master` (contiene già PR #103).
- STATUS: MERGED — PR #104, merge commit `10ec8867cc44361aa6a58db8672f14afe1359d92`, su esplicita istruzione del Product Owner ("Mergia").

- **Indagine**: verificato via Vercel MCP che entrambi i deploy di POL-UI-039 (PR #102 e #103) sono `READY` in produzione — non un fallimento di build. Ogni pagina di `App.jsx` (Agenda inclusa) è caricata con `React.lazy(() => import(...))`, dentro un UNICO `<Suspense fallback={<LoadingScreen />}>` — e `LoadingScreen.jsx` mostra letteralmente il testo "Caricamento dati…", che combacia quasi alla lettera con "mi va in caricamento". **Non esisteva nessun Error Boundary in tutta l'app** (verificato via grep — zero `componentDidCatch`/`getDerivedStateFromError` in `src/`). Se una `import()` dinamica fallisce per QUALSIASI motivo indipendente dal codice del componente stesso (un service worker che serve ancora un `index.html`/manifest vecchio con un hash di chunk che il deploy più recente non serve più, un chunk rimosso dalla CDN, un blip di rete proprio dopo un deploy), React Suspense non ha né un valore risolto né un gestore d'errore a cui ricadere: resta bloccato sulla schermata di caricamento PER SEMPRE — esattamente "non carica, va in caricamento" — e questo sopravvive a una riapertura normale perché è la cache del browser/service worker a essere disallineata, non lo stato in memoria di React.
- **Fix**: due pezzi, applicati a TUTTE le 15 pagine lazy-caricate di `App.jsx` (non solo Agenda, stesso meccanismo per tutte):
  1. Nuovo `src/lib/lazyWithRetry.js` — `importWithRetry(factory, chunkName)` (funzione pura, testata direttamente) tenta l'import; se fallisce e non è già stato ritentato in questa sessione (flag in `sessionStorage`, con fallback sicuro se `sessionStorage` è bloccato), fa un vero `window.location.reload()` — una navigazione reale che ri-scarica `index.html` e ri-risolve ogni asset da zero, rompendo qualunque cache disallineata legata alla sessione corrente (stessa tecnica già collaudata dal reload di POL-UI-032 sul logout). Se fallisce ANCORA dopo il reload (un chunk davvero mancante, non un problema transitorio), lascia che l'errore emerga per davvero invece di ritentare all'infinito. `lazyWithRetry(factory, chunkName)` avvolge questo in `React.lazy`.
  2. Nuovo `src/components/RouteErrorBoundary.jsx` — un vero Error Boundary di classe, ora attorno a ENTRAMBI i blocchi `<Suspense>` rilevanti (il router di pagina principale, con `key={page}` così si azzera cambiando pagina; e il portale della scheda paziente `PatientWorkspaceBoundary`) — se anche dopo il retry l'import fallisce ancora, o un componente lancia un errore di rendering per qualunque altro motivo, ora compare un vero messaggio "Qualcosa è andato storto" con un tasto "Ricarica", mai più una rotellina che gira all'infinito senza alcuna via d'uscita.
- VALIDATION: `npm test` 782/782 (nuovo `tests/lazyWithRetryChunkLoadFailure.test.mjs`, 6 test — inclusi test comportamentali reali su `importWithRetry` con `window`/`sessionStorage` mockati: import riuscito non fa reload, import fallito fa reload UNA volta e non risolve mai la promise, un secondo fallimento sullo stesso chunk non ritenta e fa emergere l'errore vero, `sessionStorage` bloccato non impedisce comunque il reload; aggiornati due regex ormai obsoleti in `tests/attivitaCategorie.test.mjs`/`tests/poliedronHub.test.mjs` per la nuova firma `lazyWithRetry(..., 'NomeChunk')`); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — solo componenti/lib client.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — in particolare che Agenda e le altre pagine tornino a caricarsi normalmente. Nota: dato che questo fix riguarda proprio il meccanismo di caricamento, se il Product Owner è ancora su una sessione bloccata da PRIMA di questo deploy, potrebbe comunque servire un riavvio completo UNA ultima volta per ricevere il fix stesso — dopodiché qualunque futuro problema di caricamento si risolverà da solo con un reload automatico, senza passaggi manuali.

---
- TITLE: subito dopo il deploy di POL-UI-039, il Product Owner segnala che il popup ora compare troppo in alto e non scrollabile, coperto dal "dock flottante superiore" — cioè i controlli mese/settimana flottanti di Agenda, non il dock di navigazione in basso.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Il popup adesso è troppo in alto e non è scrollabile. Viene coperto dal dock flottante superiore").
- BRANCH: `fix/agenda-popup-top-overlay-clearance`, da `master` (contiene già PR #102).
- STATUS: MERGED — PR #103, merge commit `acc6fc6b17e4771df4a8245a510323774e536465`, su esplicita istruzione del Product Owner ("Mergia").

- **Causa radice, confermata empiricamente in un repro headless-Chromium reale che include anche il vero elemento dei controlli flottanti superiori (non solo il dock)**: il fix POL-UI-039 aveva aggiunto `align-items: center` al menu contestuale, ma il suo backdrop aveva un `padding-top` piccolissimo (~12px, solo safe-area) contro un `padding-bottom` grande (~148-160px, la clearance dock/FAB) — `align-items:center` centra DENTRO quel riquadro con padding, quindi con un padding così asimmetrico il foglio veniva spinto verso l'alto, proprio nella stessa zona di schermo dei controlli flottanti mese/striscia settimana di Agenda (`.agenda-mobile-floating-controls`) — la stessa identica zona che il modulo "Modifica appuntamento" già proteggeva da tempo (POL-UI-030) ma che il menu contestuale non aveva mai protetto, perché prima di POL-UI-039 il menu non veniva mai centrato (era ancorato in basso, quindi la sua parte alta non arrivava mai così in su).
- **Fix**: `Agenda.jsx` — il backdrop del menu contestuale ora passa anche `--agenda-mobile-overlay-clearance-menu` con la stessa misura reale `mobileOverlayHeight` (lo stesso `ResizeObserver` già usato per il modulo). `PremiumVisualSystem.css` — `padding-top` del backdrop e `max-height` del foglio del menu contestuale ora sottraggono anche questa misura, non solo la safe-area, riservando spazio reale su ENTRAMBI i lati (controlli flottanti in alto, dock/FAB in basso) prima di centrare — così il centraggio avviene nello spazio libero reale tra i due elementi flottanti, non in un riquadro sbilanciato che spinge il foglio verso l'alto.
- **Riverificato a misura DOPO il fix** (stesso repro, ora con l'elemento reale dei controlli flottanti superiori incluso): su un viewport 390×700 con controlli flottanti alti 120px, il foglio ora sta a `top:133px / bottom:541px` — nessuna sovrapposizione né con i controlli in alto (`bottom:120px`) né con il dock in basso (`top:620px`), e il suo centro (337px) è vicinissimo al centro dello spazio libero reale tra i due (370px). Lo scroll interno resta necessario e funzionante: `actions.scrollHeight` (437) supera `actions.clientHeight` (349), e il tasto "Elimina" è raggiungibile solo dopo aver scrollato.
- VALIDATION: `npm test` 776/776 (nuovo test in `tests/agendaMobilePopupDockCenter.test.mjs`, ora 6 test totali — verifica che la nuova variabile di clearance superiore sia passata da `Agenda.jsx` e usata sia nel `padding-top` del backdrop sia nel `max-height` del foglio); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — solo CSS/componente client.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-UI-039 — Agenda mobile: popup appuntamento non scrollabile e coperto dal dock flottante
- TITLE: il Product Owner segnala che, toccando un appuntamento su mobile, il popup che compare non è scrollabile e appare sotto il dock flottante — chiede che compaia centrato sullo schermo e scrollabile, così da poter selezionare tutti i tasti.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "In agenda mobile quando clicco su appuntamento compare il popup ma non scrollabile e al di sotto del dock flottante , deve comparire in centro schermo e scrollabile così di possono selezionare i vari tasti").
- BRANCH: `fix/agenda-mobile-appointment-popup-dock-center`, da `master` (contiene già PR #101).
- STATUS: MERGED — PR #102, merge commit `569174e327b85b06b6125bb460492dd1fa7c7e02`, su esplicita istruzione del Product Owner ("Mergia").

- **Causa radice, confermata empiricamente in un repro headless-Chromium reale (non solo lettura del sorgente)**: `.agenda-appointment-menu-backdrop`/`.agenda-appointment-menu-sheet` (il menu contestuale che si apre toccando un appuntamento nella vista a griglia) erano selettori a UNA sola classe — perdono sempre in specificità contro le regole base `html .pol-modal-backdrop`/`html .pol-modal-sheet` di `designTokens.css` (elemento+classe batte una classe sola), e quella del foglio è anche `!important`. Risultato: il `padding-bottom` reale del backdrop (pensato per riservare spazio al dock/FAB) era sempre `0px`, e il `max-height` reale del foglio era sempre la formula generica `100dvh - space-2` — la clearance dock non si applicava MAI, il foglio arrivava a filo del bordo inferiore reale dello schermo, proprio dove vive il dock flottante. Verificato leggendo `getComputedStyle` in un vero Chromium headless (stessa metodologia già stabilita per bug di questa classe in questo codebase — un'ipotesi precedente su un `min-width:0` mancante era stata analogamente smentita così in un giro precedente), non solo ipotizzato dal sorgente. Lo stesso identico gap esisteva, per un motivo diverso, nel modulo "Modifica appuntamento" (vista Elenco, `apriEdit` diretto): la sua clearance dedicata (POL-UI-030) riservava SOLO spazio in alto (contro i controlli flottanti mese/settimana), mai in basso contro il dock — non un bug di specificità qui, semplicemente mai implementato.
- **Fix**: `PremiumVisualSystem.css` — selettori del menu contestuale promossi a coppia di classi (`.pol-modal-backdrop.agenda-appointment-menu-backdrop`, `.pol-modal-sheet.agenda-appointment-menu-sheet`) con `!important` sul `max-height`, stessa tecnica già usata con successo da POL-UI-030 per il foglio del modulo. Aggiunta clearance reale in basso anche al modulo "Modifica appuntamento" (`padding-bottom`/`max-height` che sottraggono `max(--agenda-mobile-dock-offset, --agenda-mobile-fab-offset)`, stesse costanti reali già passate al menu contestuale — `Agenda.jsx` ora le passa anche al `backdropStyle` del modulo). **"Centro schermo"**, richiesta esplicita del Product Owner: entrambi i popup ora usano `align-items: center` sul backdrop invece di `flex-end` (ereditato dalla regola base, ancoraggio in basso) — più `width`/`margin-inline` per un vero riquadro centrato con margine ai lati, non più un foglio edge-to-edge ancorato in basso. Lo scroll interno (`.agenda-appointment-menu-actions { overflow-y:auto }` e il generico `.pol-modal-content { overflow-y:auto }` per il modulo) era già corretto di suo — il problema era che, con la clearance dock morta, il contenuto raramente arrivava davvero a eccedere lo spazio disponibile, quindi lo scroll non serviva mai o restava impercettibile; ora che l'altezza disponibile è correttamente più piccola, lo scroll interno entra in gioco quando serve davvero (riverificato empiricamente: su un viewport realistico più basso, `actions.scrollHeight > actions.clientHeight` è vero e il tasto "Elimina" diventa raggiungibile solo dopo `scrollTop`).
- **Riverificato a misura DOPO il fix** (stesso repro, stesso hit-test di `document.elementFromPoint`): il foglio del menu contestuale ora sta a `top:28px / bottom:524px` su un viewport 390×700 (nessuna sovrapposizione con il dock, che sta a `620-684px`); il modulo "Modifica appuntamento" ha `max-height` reale di 528px (formula corretta: `700 - 148 - 24`, non più i 692px della formula generica morta); su un viewport più basso (375×560, per forzare davvero l'overflow) il tasto "Elimina" diventa raggiungibile solo dopo aver scrollato l'area interna — conferma che lo scroll ora serve ed effettivamente funziona.
- VALIDATION: `npm test` 775/775 (nuovo `tests/agendaMobilePopupDockCenter.test.mjs`, 5 test — verifica la specificità dei selettori corretti, il centraggio di entrambi i popup, la nuova clearance in basso del modulo, il wiring delle costanti in `Agenda.jsx`, e che lo scroll del menu contestuale resti intatto; aggiornato un commento non più accurato in `tests/mobileShell.test.mjs`, nessuna assert cambiata); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — solo CSS/componente client.
- EXACT NEXT ACTION: mergiato. Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-UI-038 — PWA: fallback di rilevamento aggiornamenti indipendente dal service worker
- TITLE: dopo POL-UI-035 (fix skipWaiting/clientsClaim), il Product Owner segnala ancora "Non va per favore sistema che ho perso tempo già troppo" — su iOS/WebKit, home-screen-installed PWA, il rilevamento aggiornamenti basato sul ciclo di vita del service worker può semplicemente non scattare mai, indipendentemente da skipWaiting/clientsClaim.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Non va per favore sistema che ho perso tempo già troppo").
- BRANCH: `fix/pwa-index-html-version-poll-fallback`, da `master` (contiene già PR #101).
- STATUS: NON MERGED — pushato, in attesa di "Mergia" esplicita del Product Owner (la conversazione è passata ad altri argomenti prima che arrivasse l'istruzione di merge).

- **Fix**: nuovo effetto in `App.jsx`, completamente indipendente dal ciclo di vita del service worker — ri-scarica `/index.html` (già no-cache per `vercel.json`) e confronta il nome file dello script principale con hash che Vite vi inietta contro quello effettivamente in esecuzione, al mount + a ogni `visibilitychange`-a-visibile + un intervallo periodico di 10 minuti. Un mismatch riusa lo stesso banner non distruttivo tap-to-update già esistente (mai un reload silenzioso).
- VALIDATION: `npm test` verde (nuovo `tests/versionPollingFallback.test.mjs`, 4 test — inclusa una verifica che la regex di estrazione parsi davvero il markup `<script>` reale di Vite, non solo un grep sul sorgente); `npm run build` pulito. Nessuna migration.
- EXACT NEXT ACTION: in attesa di "Mergia" esplicita del Product Owner.
...
- **Fix**:
  1. `vite.config.js`: aggiunto esplicitamente `workbox: { skipWaiting: true, clientsClaim: true, ... }`, ripristinando il comportamento che `registerType:'autoUpdate'` dovrebbe avere di suo. Verificato buildando di nuovo: `dist/sw.js` ora chiama `self.skipWaiting(),s.clientsClaim()` incondizionatamente all'avvio, prima di `precacheAndRoute`.
...
- TASK: POL-UI-034 — Attività e promemoria: categorie con etichette visibili + sezione dedicata unica
- TITLE: il Product Owner chiede che le Attività (e i promemoria) siano classificate con etichette visibili (es. "ordinare", "anamnesi mancante", "piano", "dati mancanti" ecc.) e che esista UNA sezione dedicata che copra entrambe, per maggiore chiarezza.
- OWNER: CLAUDE, su istruzione diretta del Product Owner. Messaggio iniziale (verbatim): "Attività e promemoria: dobbiamo mettere in modo che siano classificate con etichette visibili : tipo ordinare, anamnesi mancante , piano, dati mancanti ecc, e poi deve esserci una sezione attività in modo che sia tutto più chiaro". Dopo aver visto una prima versione solo-Attività, chiarimento (verbatim): "Deve esserci sezione apposita per attività e promemoria, ovvero pagina in cui ci siano tutte le attività per etichette implementabili" — cioè UNA pagina che copra sia Attività sia Promemoria, non solo le Attività.
- BRANCH: `feature/pol-ui-034-attivita-categorie`, da `master` (contiene già PR #98).
- STATUS: MERGED — PR #99, merge commit `160d659fe63fc046a126147b84ba8a1fb5f46c93`, su esplicita istruzione del Product Owner ("Mergia in master fino a dove hai fatto poi vediamo").

- **Indagine preliminare**: `ACTIVITY_KIND` (`src/lib/domain/dataHealthActivities.js`) aveva già 5 valori usati dal controllo dati automatico, ma il campo `entry.kind` veniva calcolato e poi scartato prima di salvare la riga in `todos` — nessuna colonna `categoria` esisteva sulla tabella. Esistevano già DUE mappe etichetta/icona parziali e duplicate tra loro (`Dashboard.jsx` `DATA_HEALTH_KIND_LABEL` per il digest in Chat Poliedron — frasi lunghe, lasciata invariata perché serve a uno scopo diverso — e `PoliedronHub.jsx` `DATA_HEALTH_KIND_TITLE`/`DATA_HEALTH_KIND_ICON`, solo 2 dei 5 kind). `RICHIAMO_CATEGORIE`/`Richiami.jsx` era il pattern di riferimento già rodato e funzionante da replicare (oggetto id→{label,icona,colore}, badge `<Ic/><Bdg/>`, tabbar filtro categoria).
- **Fix**:
  1. Migration additiva (`supabase/migrations/20260909120000_pol_ui_034_todos_categoria.sql`, applicata anche in produzione via MCP Supabase, verificata contro lo schema reale prima e dopo): `ALTER TABLE public.todos ADD COLUMN IF NOT EXISTS categoria text;` — nessuna modifica RLS necessaria (stessa policy ALL-command già scoperta per `paziente_id` in POL-FIN-007b).
  2. `TODO_CATEGORIE` in `src/lib/utils.js`, stessa forma di `RICHIAMO_CATEGORIE`: le prime 5 chiavi coincidono deliberatamente con i valori di `ACTIVITY_KIND` (zero traduzione — il controllo dati scrive `categoria: entry.kind` così com'è), più 4 chiavi solo-manuali (`DA_ORDINARE`, `DATI_MANCANTI`, `AMMINISTRATIVO`, `GENERICO`) per coprire gli esempi del Product Owner non già coperti dal controllo dati automatico.
  3. `Dashboard.jsx`: l'insert automatico del controllo dati ora include `categoria: entry.kind`; il modale "+ Nuova attività" ha un nuovo `<Fld label="Categoria"><Sel>` (stato `todoCategoria`, default `GENERICO`) incluso nell'insert manuale; il widget Attività in Home mostra ora un badge colorato (`<Ic/><Bdg/>`) sotto ogni riga che ha una categoria.
  4. `PoliedronHub.jsx`: le due mappe locali duplicate (`DATA_HEALTH_KIND_TITLE`/`ICON`) sono state rimosse — legge ora `TODO_CATEGORIE[kind].label`/`.icona` come unica fonte, eliminando la duplicazione trovata in fase di indagine.
  5. Pagina `src/components/Attivita.jsx` — **una sola pagina con DUE sezioni**, scelte con un interruttore in cima (stesso linguaggio visivo del toggle Da-fare/Fatte): "Attività" (mirror di quanto già in Dashboard/Home: StatCard Da fare/Completate, tabbar filtro `TODO_CATEGORIE`, toggle Da fare/Fatte, card con badge categoria, CRUD diretto su `supabase.from('todos')`) e "Promemoria" (stessa identica logica di `Richiami.jsx` — StatCard Scaduti/Prossimi 30gg, tabbar filtro `RICHIAMO_CATEGORIE`, Scansiona ora/bot, stesso stato `richiami`/`setRichiami` di `App.jsx`, nessun dato duplicato). Il vecchio ingresso "Richiami" in NAV resta invariato e pienamente funzionante (stesso principio già usato per il tasto Esci duplicato in POL-UI-031/033: un secondo punto d'accesso allo stesso stato, non una copia dei dati). Nuova voce `NAV` (`{ id: 'attivita', l: 'Attività', ic: 'clip' }`, ungated come `richiami`) e route lazy in `App.jsx` (ora passa anche `richiami`, `setRichiami`, `plans`, `payments`, `appointments`, `templates`, `features`, `si`); aggiunta anche a `DEF_DOCK_SETTINGS.menuItems` accanto a `richiami`.
- **Aggiunta successiva**: Product Owner, dopo aver visto la pagina Attività/Promemoria: "Dobbiamo mettere il tasto su menu poliedron per attività altrimenti non si trova" — `PoliedronHub.jsx` (`TABS`) aveva già il pattern "external" per un tasto che naviga fuori dalla sezione invece di cambiare tab interna (usato da "Chat"), ma con la destinazione `'chat'` hardcoded nell'handler. Generalizzato con un campo `page` per voce (`{ id: 'attivita', icon: 'clip', label: 'Attività', external: true, page: 'attivita' }`, e `{ id: 'chat', ..., page: 'chat' }`), `handleTabClick` ora fa `onNavigate(item.page)` invece di `onNavigate('chat')` fisso — stesso `onNavigate={setPage}` già passato da `App.jsx`, nessuna nuova logica di routing.
- **Aggiunta successiva 2**: Product Owner ("Non c'è" sul tasto Poliedron — quasi certamente perché stava guardando l'app di produzione, che non ha ancora questo branch non mergiato — "comunque metti anche il widgets cliccabile che porti alla pagina attività"): il widget Home "Attività e promemoria" (`Dashboard.jsx`, blocco `w.id === 'todo'`) ora ha un link "Vedi tutte ›" nell'header della sezione, stesso pattern già usato dal widget "Azioni rapide" (`className="home-list-link"`, `onClick={() => onNavigate('attivita')}`) — un terzo punto d'accesso alla stessa pagina, oltre a NAV e al tasto in Poliedron.
- **Aggiunta successiva 3**: Product Owner: "la visualizzazione di attività in home deve essere di una riga più etichetta, ordinato e bello da vedere, poi devono essere cliccabili". Ridisegnata la riga del widget Home `w.id === 'todo'`: testo troncato a una riga (ellipsis, non va più a capo) + badge categoria affiancato sulla stessa riga invece che sotto; l'intera riga (non solo il testo) è ora un `<button>` cliccabile con freccia "›" finale — se la riga ha un paziente associato apre quel paziente (come già prima), altrimenti (nessun paziente — prima non era cliccabile affatto) porta alla pagina Attività dedicata, dove c'è più contesto per gestirla.
- **Domanda aperta del Product Owner, non ancora implementata**: "il problema è che poi essendoci tante attività si perdono se non si completano tutte, come facciamo?" — causa individuata: `loadTodos()` ordina per `created_at` decrescente (più recenti prima) e non viene mai riordinato, quindi via via che si accumulano nuove Attività quelle vecchie non completate scivolano in fondo alla lista scrollabile (max 220px) del widget Home e si perdono di vista. Proposta (in attesa di conferma, non implementata): ordinare per urgenza invece che per data di creazione — es. le più vecchie/scadute per prime, o evidenziarle con un badge "in attesa da N giorni" simile allo "scaduto" già usato nei Promemoria — così restano in cima finché non vengono chiuse, invece di sparire sotto le nuove.
- VALIDATION: `npm test` 757/757 (`tests/attivitaCategorie.test.mjs` 10 test, `tests/dashboardDataHealthActivities.test.mjs` aggiornato per il nuovo handler `apriTodo`, `tests/poliedronHub.test.mjs` aggiornato per il nuovo tasto Attività); `npm run build` pulito; `git diff --check` pulito. Migration applicata in produzione via MCP Supabase (verificata `information_schema.columns` prima e dopo).
- EXACT NEXT ACTION: mergiato ("Mergia in master fino a dove hai fatto poi vediamo"). Resta in sospeso la domanda aperta sopra (ordinamento/evidenziazione delle Attività vecchie non completate) — in attesa che il Product Owner risponda con una direzione prima di implementarla.
...
- BRANCH: `fix/logout-forces-reload-for-updates` (stesso branch/PR #98 del giro precedente POL-UI-032).
- STATUS: MERGED — vedi PR #98 registrata nell'entry POL-UI-032 sotto (stesso branch, stesso PR, unico merge per entrambi i giri).
...
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — vedi comunque la nota sul riavvio completo nell'entry POL-UI-032 sotto: serve una prima volta, fatta fuori dall'app, prima che questi due fix stessi diventino visibili.
...
- STATUS: MERGED — PR #98, merge commit `fdc9dde5707ec88f496b3657fc4372c2e8b54ee5`, su esplicita istruzione del Product Owner ("Mergia").
...
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — poi dovrà comunque forzare UN riavvio completo dell'app (chiudi del tutto e riapri, o disinstalla/reinstalla la PWA) per ricevere questo stesso fix, dato che è il fix del meccanismo di aggiornamento stesso e non può auto-applicarsi finché non è la prima volta attivo.
...
- TASK: POL-UI-030 — Fix: modulo "Modifica appuntamento" (POL-UI-027) ora coperto dai controlli flottanti mese/settimana in alto
- TITLE: la correzione precedente (POL-UI-027, `mobileVariant="sheet"`) fa arrivare il modulo quasi a tutta l'altezza dello schermo per liberare il dock in basso — ma così facendo la sua parte alta finisce nella stessa zona di schermo occupata dai controlli flottanti mese/striscia settimana di Agenda, dando l'impressione che il popup venga coperto da quelli.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Adesso compare troppo in alto il popup di modifica appuntamenti quindi viene coperto da giorni settimana flottanti , prendi bene le misure").
- BRANCH: `fix/pwa-update-reload-prompt` (stesso branch/PR #96 del giro precedente).
- STATUS: MERGED — vedi PR #96 registrata nell'entry POL-UI-029 sotto (stesso branch, stesso PR, unico merge per entrambi i giri).

- **Verifica "a misura" (non solo teoria)**: richiesta esplicita del Product Owner di "prendere bene le misure" — costruita una seconda repro isolata (CSS reale, servita via HTTP) che include ANCHE i controlli flottanti reali di Agenda (`.agenda-mobile-floating-controls`, `position:absolute`, `z-index:30`, dentro `.agenda-mobile-grid-surface` che è `position:relative;z-index:0` — quindi il suo z-index:30 resta comunque dentro un contesto di stacking con z-index effettivo 0, sotto i 9999 del modale: in teoria già coperto). Campionamento a griglia con `document.elementFromPoint()` su tutta l'area dei controlli flottanti (prima del fix): conferma che il modale li copre già correttamente (mai un hit sui controlli), ma le MISURE reali mostravano il bordo superiore del foglio a soli ~pochi px dall'alto — visivamente a ridosso della fascia dei controlli flottanti (~122px di altezza reale, month pill + week strip), anche se tecnicamente "sotto" il modale. Non un bug di z-index, ma di spazio: nessuna delle due varianti `mobileVariant` conosce l'esistenza di quei controlli.
- **Fix**: `ui/Modal.jsx` — nuovi prop opt-in `backdropClassName`/`sheetClassName`/`backdropStyle` (default vuoti, zero effetto su ogni altro Modal esistente nell'app — verificato: `npm test` 741/741 invariato) per permettere a UNA istanza specifica di riservare spazio reale attorno al foglio, stessa tecnica già usata per il menu contestuale appuntamento (`agenda-appointment-menu-backdrop`/`-sheet`), ora resa riusabile invece di restare un markup a sé. In `Agenda.jsx`, il modale "Nuovo/Modifica appuntamento" ora passa `--agenda-mobile-overlay-clearance-form` con l'altezza REALE MISURATA dei controlli flottanti (`mobileOverlayHeight`, lo stesso `ResizeObserver` già usato per lo spaziatore di scroll in alto della griglia — non una costante indovinata, la stessa identica misura live già affidabile altrove in questo file). Nuove regole in `PremiumVisualSystem.css`: `padding-top` sul backdrop e `max-height` ridotta sul foglio, entrambe pari a quella misura + piccolo margine; selettori a doppia classe (`.pol-modal-backdrop.agenda-appointment-form-backdrop` / `.pol-modal-sheet.agenda-appointment-form-sheet`) per battere in specificità le regole base `html .pol-modal-backdrop`/`html .pol-modal-sheet` di `designTokens.css` (necessario, non un dettaglio: una singola classe nuova avrebbe perso silenziosamente).
- **Riverificato a misura DOPO il fix**: stessa repro, stesso hit-test — con la nuova clearance il bordo superiore del foglio ora sta a 368px dall'alto (controlli flottanti alti 122px, quindi ~246px di margine reale, mai un hit sui controlli su tutta la loro area) E il dock in basso resta correttamente coperto su tutta la sua area (nessuna regressione al fix POL-UI-027).
- VALIDATION: `npm test` 741/741 (nuovo test in `tests/mobileShell.test.mjs`: verifica i nuovi prop di `Modal.jsx`, il wiring in `Agenda.jsx` con la misura reale, e che i selettori CSS abbiano la specificità corretta per vincere sulle regole base); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — solo componenti/CSS client.
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-UI-029 — PWA: le versioni pubblicate non arrivavano mai a una sessione già aperta (nessun aggiornamento/reload)
- TITLE: il Product Owner ha segnalato che il fix POL-UI-027 (dock che copre i tasti in "Modifica appuntamento") sembrava "ancora rotto" — investigazione approfondita (repro isolata con la CSS reale + hit-test via `elementFromPoint`, non solo lettura del codice) ha CONFERMATO che il fix è corretto e già in produzione (deployment Vercel verificato `READY`/`production`); la causa reale è che questa è una PWA e nessuna parte del codice ascoltava mai un aggiornamento del service worker — una sessione già aperta (specialmente installata su home screen, sempre "ripresa" dal background, mai davvero ricaricata) continuava a eseguire il JS vecchio all'infinito, invisibile a QUALUNQUE fix di questa sessione, non solo POL-UI-027.
- OWNER: CLAUDE, di propria iniziativa dopo aver verificato con `elementFromPoint` (non solo il codice) che il fix POL-UI-027 è corretto — su richiesta del Product Owner di "continuare" con un problema che, per lui, sembrava ancora presente ("Continua con il problema dello scorrimento quando clicco per modificare appuntamento").
- BRANCH: `fix/pwa-update-reload-prompt`, da `master` (contiene già PR #95).
- STATUS: MERGED — PR #96, merge commit `05ac8dcb608537bf3d15bfbd24a3936137e9ec6f`, su esplicita istruzione del Product Owner ("Mergia in master").

- **Verifica del fix POL-UI-027 (perché "sembrava" ancora rotto)**: creata una pagina HTML isolata che carica le VERE `designTokens.css`/`PremiumVisualSystem.css` (servite via HTTP locale, non `file://`, per evitare restrizioni cross-origin che falsavano un primo tentativo) e riproduce il DOM esatto di `Modal.jsx`+`PoliedronMobileDock.jsx`. Uno screenshot statico sembrava mostrare il dock sopra ai tasti — ma è risultato un artefatto della cattura headless (`position:fixed` con pagina scrollabile), non il rendering reale: un campionamento a griglia di `document.elementFromPoint()` su tutta l'area del dock (col body scroll bloccato, come fa davvero `Modal.jsx`) restituisce SEMPRE un elemento del modulo (`SELECT`, `btn-dan`, `btn-sec`), mai il dock — cioè il dock è realmente coperto/non cliccabile quando il modulo è aperto, esattamente come progettato. Verificato anche via Vercel MCP che l'ultimo deploy `production`/`READY` include il commit del fix. Conclusione: il fix POL-UI-027 funziona; il problema segnalato come "ancora presente" ha un'altra causa.
- **Causa reale trovata**: `vite.config.js` aveva `injectRegister: 'auto'`, che per `registerType:'autoUpdate'` genera semplicemente `if('serviceWorker' in navigator) navigator.serviceWorker.register(...)` — nessun listener di aggiornamento, nessun reload, mai. Il service worker generato (`registerType:'autoUpdate'`) fa sì `self.skipWaiting()`+`clientsClaim()` da solo non appena una nuova versione si installa, ma senza nessuno che ascolti quell'evento lato client, la pagina già aperta continua a eseguire il bundle JS vecchio indefinitamente — su una PWA installata (aperta/richiusa dalla home, mai un vero "reload"), questo può non risolversi mai da solo.
- **Fix**: `vite.config.js` → `injectRegister: false` (disabilita lo script auto-iniettato); `App.jsx` registra il service worker esso stesso via `virtual:pwa-register`, con `onNeedReload` (il callback corretto per `registerType:'autoUpdate'` — richiamato quando il nuovo service worker ha GIÀ preso il controllo, mai prima) che mostra un banner ("🔄 Nuova versione disponibile — tocca per aggiornare"), stesso linguaggio visivo/safe-area-top del banner `syncError` (POL-UI-028) appena corretto. Il reload NON è mai automatico/silenzioso (rischio di perdere un form a metà) — solo al tocco dell'utente, o ignorabile con "✕" (si applicherà comunque al prossimo riavvio naturale dell'app).
- VALIDATION: `npm test` 740/740 (nuovo `tests/pwaUpdatePrompt.test.mjs`); `npm run build` pulito (verificato anche manualmente: `dist/index.html` non contiene più il vecchio script auto-iniettato, il bundle contiene la nuova logica `onNeedReload`, il chunk `workbox-window` viene generato correttamente); `git diff --check` pulito. Nessuna migration — solo configurazione build + componente client.
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — poi dovrà comunque forzare UN riavvio completo dell'app (chiudi del tutto e riapri, o disinstalla/reinstalla la PWA) per ricevere questo stesso fix, dato che è il fix del meccanismo di aggiornamento stesso e non può auto-applicarsi finché non è la prima volta attivo.

---

- TASK: POL-UI-027 — Fix: modulo "Modifica appuntamento" non scorreva fino ai tasti, coperti dal dock mobile
- TITLE: in Agenda, aprendo "Modifica appuntamento" da mobile, il form (11 campi) non arrivava a mostrare i tasti Elimina/Annulla/Aggiorna in fondo — restavano coperti dal dock di navigazione flottante.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "In agenda quando clicco appuntamento per modificare non scorre fino al di sopra del dock quindi alcuni tasti sono coperti dal dock , sistema").
- BRANCH: `fix/agenda-edit-modal-dock-scroll`, da `master` (contiene già PR #93; riallineato a `origin/master` — che nel frattempo aveva già ricevuto PR #94, riportata di seguito — su istruzione esplicita del Product Owner "Mergia anche il precedente 027").
- STATUS: MERGED — PR #95, merge commit `25e90e3889162d7c955fc97209d6c62d18471aba`, su esplicita istruzione del Product Owner.

- **Causa radice**: il modulo "Nuovo/Modifica appuntamento" (`Agenda.jsx`) usa il `Modal` condiviso (`ui/Modal.jsx`) senza specificare `mobileVariant`, quindi riceve il default `"standard"` — che su mobile riserva solo un piccolo `margin-bottom` (`max(--pol-space-2, safe-area)`, pochi px) sotto il foglio (`designTokens.css`). Il dock mobile flottante (`.poliedron-mobile-dock`, `position:fixed`, ~80-96px di altezza reale incluso lo spazio da terra) è molto più alto di quel margine: il suo ingombro reale finisce per sovrapporsi al fondo del foglio invece di stare nel piccolo spazio libero sotto — esattamente dove terminava questo form lungo (Paziente/Data/Ora/Durata/Stato/Tipo/Operatore/Poltrona/Note/Ripeti + i tre tasti finali). Confrontato con gli altri moduli con contenuto ugualmente lungo e un tasto finale (`IncassoModal`, "Registra da incassare", "Allega foto o PDF", `ProdottoReconciliationModal`) — tutti usano già `mobileVariant="sheet"`, che NON ha una regola dedicata in `designTokens.css` (solo `"standard"`/`"fullscreen"` ce l'hanno) e quindi eredita la regola base: nessun margine riservato, il foglio arriva fino al bordo reale dello schermo, coprendo per intero il dock (z-index del modale 9999 contro 1100 del dock, più `overflow:hidden` sul foglio) — nessuno spazio scoperto in cui il dock possa sovrapporsi.
- **Fix**: una riga — `mobileVariant="sheet"` aggiunto al `<Modal>` di "Nuovo/Modifica appuntamento" in `Agenda.jsx`, stesso valore già usato con successo per moduli strutturalmente identici. Nessuna nuova CSS, nessun nuovo prop sul componente `Modal` condiviso — riuso di una variante già esistente e già testata dagli altri moduli.
- **Non toccato in questo giro**: il modulo "Nuovo/Modifica impegno personale" (`Agenda.jsx`, stesso file) ha un form di lunghezza comparabile e lo stesso default `mobileVariant="standard"` — probabile stesso bug latente, ma non segnalato esplicitamente dal Product Owner in questo messaggio; fix identico disponibile su richiesta.
- VALIDATION: `npm test` 736/736 (nuovo test in `tests/mobileShell.test.mjs`: verifica `mobileVariant="sheet"` sul modulo appuntamento e che quella variante non abbia un override dedicato — cioè eredita davvero il comportamento "arriva al bordo" già provato altrove); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — solo un prop su un componente client.
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-UI-028 — Fix: salvataggio impegno personale falliva sempre + banner errore sotto la status bar
- TITLE: aggiungere un impegno con orario specifico in Agenda falliva SEMPRE il salvataggio sul cloud ("Could not find the 'recurrenceId' column of 'impegni_personali' in the schema cache"); il banner di errore, inoltre, si vedeva a malapena/sembrava "non togliersi più" perché renderizzato sotto la status bar del telefono su mobile.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim, con screenshot: "Quando. Aggiungo impegno in agenda mi da errore di salvataggio e non si toglie più l'avviso , togli avviso , sistema la cosa che si possa memorizzare un impegno").
- BRANCH: `fix/impegno-recurrenceid-save-error`, da `master` (contiene già PR #93; NON contiene ancora il fix POL-UI-027 del dock in Agenda, ancora sul suo branch separato non mergiato).
- STATUS: MERGED — PR #94, merge commit `e2770e16b0d345706ad5f6d6b8ed7be010b0f49e`, su esplicita istruzione del Product Owner ("Mergia in master").

- **Causa radice #1 (verificata sul DB reale prima di scrivere qualunque fix)**: `Agenda.jsx`'s `saveImpegno()` include SEMPRE una chiave `recurrenceId` su ogni impegno con orario specifico (anche `null` per un impegno senza ripetizione — la chiave resta comunque presente nell'oggetto, PostgREST rifiuta l'insert indipendentemente dal valore se la colonna non combacia). Verificato via `information_schema.columns` che `impegni_personali` ha REALMENTE una colonna `recurrence_id` (uuid) — non mancante, semplicemente mai mappata: `src/lib/supabase.js`'s `FIELD_MAP.impegni_personali` non aveva la conversione camelCase→snake_case per questo campo (aveva `dataInizio`/`dataFine`/`tuttoIlGiorno`/`oraInizio`/`oraFine` ma non `recurrenceId`), quindi `toDb()` mandava la chiave grezza `recurrenceId` a Supabase, che non esiste con quel nome → fallimento SEMPRE, per ogni impegno con orario (il percorso "tutto il giorno" non include questo campo ed era l'unico che funzionava). Confermato che l'`id` di raggruppamento non è mai letto da nessuna parte del codice oggi (`delImpegno` cancella solo il singolo id) — la mappatura mancante era l'unico problema, non uno schema da correggere.
- **Fix #1**: una riga — `recurrenceId: 'recurrence_id'` aggiunta a `FIELD_MAP.impegni_personali` in `src/lib/supabase.js`. Corregge sia scrittura (`toDb`) sia lettura (`fromDb`, che deriva la mappa inversa dalla stessa tabella) con un solo punto di modifica. Nessuna migration necessaria — la colonna esisteva già.
- **Causa radice #2 ("l'avviso non si toglie")**: il banner `syncError` in `App.jsx` (quello con "⚠️ ... ✕") è renderizzato PRIMA del contenuto di pagina, fuori da qualunque gestione del safe-area-top — a differenza di ogni altra superficie mobile dell'app (che riserva esplicitamente `env(safe-area-inset-top)`, vedi il `paddingTop` della pagina poche righe sotto), quindi su un telefono con status bar/notch il banner (testo e tasto "✕") si sovrapponeva visivamente alla status bar di sistema, rendendo il tasto di chiusura difficile da toccare con precisione — da qui la sensazione che l'avviso "non si tolga più" (in realtà si chiude, l'ogni-volta-fallito salvataggio del Fix #1 lo faceva anche ripresentare ad ogni tentativo).
- **Fix #2**: `padding-top` del banner ora `calc(9px + env(safe-area-inset-top, 0px))` su mobile (era `9px` fisso su ogni piattaforma), stesso pattern già usato per il `paddingTop` della pagina.
- VALIDATION: `npm test` 737/737 (nuovo `tests/agendaImpegnoSave.test.mjs`: verifica la mappatura `recurrenceId`→`recurrence_id` e il safe-area-top del banner); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — colonna `recurrence_id` già esistente in produzione, solo mappatura client + CSS corretti.
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge) — in particolare, che un impegno con orario specifico si salvi davvero ora.
...
- STATUS: MERGED — PR #93, merge commit `da7188c8e2eb4988745b694b0a37754c469c3781`, su esplicita istruzione del Product Owner ("Mergia in master"). Branch riallineato a `origin/master` (che nel frattempo aveva già ricevuto PR #92) prima del merge.
...
- TASK: POL-UI-026 follow-up — Andamento studio unito a Preventivi, nuovi pazienti cliccabili
- TITLE: dopo aver visto i due widget Home separati (Andamento studio, Preventivi), il Product Owner ha chiesto di unirli in un'unica card "Andamento studio" — nuovi pazienti in alto (i numeri devono aprire l'elenco dei pazienti reali), preventivi sotto, con un design più curato.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "Andamento studio deve essere unito a preventivi e chiamarsi andamento studio così indichi i pazienti nuovi (che devono essere elencati cliccando il numero), e indichi quindi il resto : preventivi ecc deve essere bello e pro").
- BRANCH: `feature/pol-ui-026-andamento-studio-nuovi-pazienti` (stesso branch/PR #92).
- STATUS: MERGED — PR #92, merge commit `b6f15b387b14b877c483dc6d255aa65b3ffbd85f`, su esplicita istruzione del Product Owner.

- **Widget `preventivi` rimosso dal registro** (`homeWidgetRegistry.js`) — non solo nascosto, stesso meccanismo sicuro già usato in POL-UI-025 per i widget Poliedron (`normalizeHomeLayout` scarta gli id sconosciuti nei layout già salvati). Il suo contenuto (Accettati/In attesa/Rifiutati) si è spostato dentro la stessa card di `andamento_studio`, che passa a `defaultSize: 'wide'` (più contenuto della card originale).
- **Dashboard.jsx — una sola card**: "Pazienti nuovi" (Questo mese / Quest'anno, cliccabili) in alto, un separatore, poi "Preventivi" (Accettati/In attesa/Rifiutati + percentuale di accettazione) sotto — tutto dentro un unico `<Crd>`. Colori dei tre preventivi allineati a quelli già usati per lo stesso identico terzetto in `Pazienti.jsx` (Accettati=verde/successo, In attesa=ambra, Rifiutati=rosso) invece del viola/accento generico del vecchio widget isolato — stessa metrica, stesso significato visivo ovunque nell'app.
- **Nuovi pazienti cliccabili**: click su "Questo mese"/"Quest'anno" apre lo stesso pattern `detailModal` già usato per Accettati/In attesa/Rifiutati/Scadenze/ecc., con l'elenco reale dei pazienti (nome cliccabile → apre la scheda, tab Info), ordinati dal più recente. Nuovo helper `pazientiNuoviIn(patients, prefissoData)` in `utils.js` (stesso predicato di `contaPazientiNuovi`, ora esposto anche come lista e non solo come conteggio — un'unica definizione, mai due implementazioni che potrebbero disallinearsi).
- VALIDATION: `npm test` 725/725 (aggiornato `tests/andamentoStudioNuoviPazienti.test.mjs` con due nuovi test — rimozione di `preventivi`, click-through/colori semantici del widget unito; aggiornati `tests/mobileHomeRound2.test.mjs`, `tests/homeLayoutVerifiedPersistence.test.mjs` per riflettere la rimozione dell'id dal registro); `npm run build` pulito; `git diff --check` pulito. Nessuna migration.
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).

---

- TASK: POL-UI-026 — Fix "Andamento studio" (nuovi pazienti sempre a zero) + nuovo widget Home
- TITLE: la sezione "Andamento studio" in cima a Pazienti.jsx (nuovi pazienti mese/anno) mostrava sempre zero, in ogni studio, da sempre — causa radice trovata e corretta alla fonte (non solo sullo schermo segnalato); la stessa metrica ora esiste anche come widget Home opzionale.
- OWNER: CLAUDE, su istruzione diretta del Product Owner (messaggio verbatim: "In scheda pazienti c'è un a parte superiore che si chiama andamento studio , indica pazienti nuovi ecc ma in realtà è sempre zero , sia mese che anno , vanno contati e segnalati i pazienti nuovi , controlla e fallo funzionare , rendil panche widget per home , visibile a d attivazione in setup").
- BRANCH: `feature/pol-ui-026-andamento-studio-nuovi-pazienti`, da `master` (contiene già PR #91).
- STATUS: MERGED — vedi PR #92 registrata nell'entry POL-UI-026 follow-up sopra (stesso branch, stesso PR, unico merge per tutti i giri accumulati).

- **Causa radice, verificata in produzione (sola lettura) prima di scrivere qualunque fix**: `Pazienti.jsx`'s `dataCreazione(p)` faceva `new Date(Number(p.id))` — ma `patients.id` è un bigint sequenziale del DB (1, 2, 3... verificato: `information_schema.columns` mostra `id bigint`, nessun default legato al tempo; righe reali `id=103 → created_at 2026-09-01`, `id=1` → primissimo paziente), non un timestamp. `new Date(103)` cade nel 1970: il confronto `startsWith(meseCorrente/annoCorrente)` (es. "2026-09") non poteva mai combaciare — zero garantito, per sempre, su ogni installazione. La vera data di creazione esisteva già nel DB (`patients.created_at`, colonna reale con default `now()`) ma `src/lib/supabase.js`'s `fromDb()` la scartava esplicitamente ad ogni lettura (`if (k === 'id' || k === 'user_id' || k === 'created_at') return;`) — non arrivava mai all'app.
- **Stesso identico bug trovato anche altrove** (stessa causa, tre schermate diverse): `useControlloDati.js`'s `nuoviMese` (che alimenta "Pazienti totali +N questo mese" sia in Dashboard.jsx sia — due volte — in PanoramicaControllo.jsx) aveva la stessa riga `new Date(Number(p.id))`. Corretto una sola volta alla fonte, si propaga automaticamente a tutti e tre gli schermi (PanoramicaControllo.jsx non è stato toccato: legge lo stesso hook).
- **Fix**: `src/lib/supabase.js` — `fromDb()` non scarta più `created_at`; mappato come `createdAt` in `FIELD_MAP.patients` (stesso meccanismo camelCase↔snake_case già usato per gli altri campi), aggiunto a `UI_ONLY_FIELDS` così non viene mai reinviato in una insert/update (è generato dal DB, mai scritto dall'app). Nuovo helper puro condiviso `contaPazientiNuovi(patients, prefissoData)` in `src/lib/utils.js` — un'unica definizione di "nuovo paziente" (mai più `p.id`), riusata da `Pazienti.jsx`, `useControlloDati.js` (che ora espone anche `nuoviAnno`, prima assente) e dal nuovo widget Home.
- **Verificato con numeri reali di produzione** (sola lettura, RLS invariata): 2 pazienti creati a settembre 2026, 72 nell'anno (coerente: lo studio è stato configurato quest'anno, quindi tutti i 72 pazienti risultano "nuovi dell'anno" — non un bug, un dato reale).
- **Nuovo widget Home `andamento_studio`** ("Andamento studio" — Nuovi questo mese / Nuovi quest'anno, cliccabile verso Pazienti): aggiunto a `homeWidgetRegistry.js` con `defaultVisible: false` — nascosto finché l'utente non lo attiva lui stesso dal pannello di personalizzazione Home (stesso meccanismo già usato per preventivi/scadenze/statistiche/ecc., "visibile ad attivazione" richiesto dal Product Owner), nessun `permission` gate (stessa platea di chi già vede la sezione in Pazienti.jsx, non solo chi ha `management_control`). Deliberatamente NON duplica i tre numeri "Preventivi accettati/in attesa/rifiutati" della sezione originale — quelli hanno già un proprio widget Home dedicato (`preventivi`), attivabile separatamente; il nuovo widget si concentra sulla metrica realmente segnalata come rotta.
- VALIDATION: `npm test` 724/724 (nuovo `tests/andamentoStudioNuoviPazienti.test.mjs`: `contaPazientiNuovi` con dati reali di esempio inclusi i casi limite, verifica che il vecchio bug `Number(p.id)`/`new Date(tms)` sia sparito da ogni punto, wiring `supabase.js`/registry/Dashboard; aggiornato `tests/mobileHomeRound2.test.mjs` per includere il nuovo id nella lista registry attesa); `npm run build` pulito; `git diff --check` pulito. Nessuna migration (colonna `created_at` già esistente, solo mai letta dall'app).
- EXACT NEXT ACTION: Product Owner verifica in produzione (Vercel farà il deploy automatico da questo merge).
