...
- TASK: POL-FIN-002-AUDIT
- TITLE: Payments/Incassi/Da-incassare architecture audit — single-source fix (Product Owner request, not yet PR'd/merged)
- OWNER: CLAUDE, on direct Product Owner request after PR #75/#76 merged: "Fai audit di funzione pagamenti, incassi, da incassare... dobbiamo collegare tutto ad in un posto quando incassiamo... e il dato deve essere uguale dovunque sia necessario".
- BRANCH: `claude/pol-ui-017-mobile-home-r2-3pizhn` (restarted from `master@2812d1c` — its previous content, POL-UI-017, was already fully merged as PR #74; same branch name reused per this session's branch convention, unrelated task)
- BASE: `origin/master@2812d1c`
- STATUS: IMPLEMENTED, NOT YET PUSHED/PR'D — awaiting Product Owner instruction to open a PR (never opened unprompted).
- AUDIT FINDING: payment creation already converges on one real container (`public.payments`, reached from 6+ UI/AI entry points through the shared `setPaymentsSync`/`DB.insert`). But a LEGACY, disconnected system also existed in parallel: a manual boolean `incassata` per plan item, toggled via "Incassata"/"Da incassare" buttons in `Piani.jsx` and `SchedaPaz.jsx`, that never touched `payments` and never moved `get_saldo_piano`/`get_saldi_aperti_studio` (POL-FIN-002's canonical formulas). That same flag fed `useControlloDati.js`'s `esegDaInc`/`totEsegDaInc` (Dashboard's "Eseguito da incassare" box, `ControlloCockpit.jsx`'s cashflow forecast, and the orphaned/unused `PanoramicaControllo.jsx`) and `richiamiBot.js`'s "eseguita e non incassata" reminder — all disconnected from the real payments/saldo data `Incassi.jsx` and `SchedaPaz.jsx`'s "Situazione finanziaria" widget already used correctly.
- PRODUCT OWNER ANSWERS (via `AskUserQuestion`, all three confirmed before implementing):
  1. "Il pulsante da incassare deve aggiornare il da incassare, incassata deve aggiornare il incassato" — the toggle must have real effect, not stay cosmetic.
  2. "Sì, aggiorna anche la proiezione" — Cockpit's cashflow forecast must also use the real saldo.
  3. "Sì, usa il saldo reale" — richiamiBot must use the real saldo instead of the manual flag.
- WHAT SHIPPED:
  - `Piani.jsx`/`SchedaPaz.jsx`: the "Incassata"/"Da incassare" button (`toggleIncassata` → `onIncassataClick`) is no longer a flag flip. Not-yet-incassata → opens the SAME quick-payment form the "Registra pagamento adesso" checkbox already uses (`openQuickPayment`); on save, `saveQuickPayment` now creates the real payment AND marks the voce `incassata: true` with a `paymentId` linking the two. Already-incassata → confirms, then deletes that same linked payment and clears `incassata`/`paymentId`. Both entry points (the button and the post-execution checkbox) now funnel through one real mechanism — no more silent divergence between the flag and the real data.
  - `useControlloDati.js`: `esegDaInc`/`totEsegDaInc` are now built from `fetchSaldiApertiStudio` (`get_saldi_aperti_studio`, `eseguito_non_pagato` field) instead of iterating `v.eseguita && !v.incassata`. Since Dashboard.jsx AND `ControlloCockpit.jsx` both consume this one hook (and `PanoramicaControllo.jsx`, confirmed unused/orphaned — not rendered anywhere in `App.jsx`/`ControlloGestione.jsx` — gets it too for free), this single change fixes the Dashboard box, the Cockpit forecast (Product Owner answer 2), and the dead component, with zero other component edits needed. Refetches whenever `plans`/`payments` change locally.
  - `richiamiBot.js`: `generaRichiamiBot` takes a new `saldiAperti` param (rows from `get_saldi_aperti_studio`, default `[]` — no existing caller broken); point 4 ("prestazioni eseguite ma non ancora incassate") now checks the plan's real `eseguito_non_pagato > 0` instead of `v.incassata`. `App.jsx` fetches `get_saldi_aperti_studio` once into a new `saldiApertiStudio` state (mirrors the existing `saldiPiani` per-patient fetch pattern) and passes it into the `generaRichiamiBot` call.
  - Incidental fix: `richiamiBot.js`'s own `import ... from './utils'` (no extension) could never be resolved by plain Node ESM (only by Vite's bundler) — latent since no test ever imported it directly before. Fixed to `'./utils.js'`; zero behavior change, required to actually test `generaRichiamiBot` for real instead of only at the source-text level.
- KNOWN BEHAVIOR CHANGE (flagged explicitly, not silently absorbed): any plan item that was manually marked "Incassata" in the OLD system without ever having had a real payment registered for it will now correctly show back up as outstanding everywhere (Dashboard, Incassi, richiami) — because the canonical `payments` table, not the old flag, is now the single source of truth. This is the intended effect of "il dato deve essere uguale dovunque", but is a visible change on any existing data that relied on the old flag alone.
- SAFETY: no migration, RLS, or schema change — `payments`/`plans` are the same tables POL-FIN-002 already writes to; no new RPC. No silent deletion of a payment: reverting "Incassata" always confirms first and only ever removes the one payment that this same flow created (tracked via `paymentId`); a payment registered any other way is never touched by this toggle.
- VALIDATION: full `npm test` 601/601 (10 new: `tests/planExecutionUi.test.mjs` extended, new `tests/richiamiBotSaldoReale.test.mjs` — real `generaRichiamiBot` calls, not just source regex — and `tests/useControlloDatiSaldoReale.test.mjs`); `npm run build` clean (only the pre-existing chunk-size warning); `git diff --check` clean.
- NOT VERIFIABLE: authenticated runtime/visual QA — same constraint as every prior round in this repo, no browser session available here.
- EXACT NEXT ACTION: committed locally, not yet pushed. Push to `claude/pol-ui-017-mobile-home-r2-3pizhn` and report to the Product Owner; open a PR ONLY if explicitly asked (not asked yet, per standing instruction). Do not merge without explicit approval.
