...
- BRANCH: `fix/pwa-update-reload-prompt` (stesso branch del giro precedente, non ancora mergiato — continua ad accumulare finché non arriva "mergia").
...
- EXACT NEXT ACTION: push del branch, apertura PR, merge solo su istruzione esplicita del Product Owner — poi il Product Owner dovrà comunque forzare UN riavvio completo dell'app (chiudi del tutto e riapri, o disinstalla/reinstalla la PWA) per ricevere questo stesso fix, dato che è il fix del meccanismo di aggiornamento stesso e non può auto-applicarsi finché non è la prima volta attivo.
