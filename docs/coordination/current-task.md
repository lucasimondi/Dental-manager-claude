...
- VALIDATION: `npm test` 756/756 (`tests/attivitaCategorie.test.mjs` 9 test + `tests/poliedronHub.test.mjs` aggiornato per il nuovo tasto Attività e per l'handler generalizzato); `npm run build` pulito; `git diff --check` pulito. Migration applicata in produzione via MCP Supabase (verificata `information_schema.columns` prima e dopo).
