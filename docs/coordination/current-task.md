...
- TASK: POL-FIN-007e — Correzione doppio incasso, scroll oltre il dock in Piani paziente, tab compatti
- TITLE: tre fix indipendenti su feedback diretto del Product Owner dopo l'uso reale dell'app: (1) un doppio incasso reale corretto nei dati + prevenuto lato app, (2) il pulsante "+ Aggiungi prestazione" in Piani paziente non era più raggiungibile su mobile perché nascosto dietro il dock flottante, (3) i tab della scheda paziente occupavano troppo spazio verticale.
- OWNER: CLAUDE, su segnalazione diretta del Product Owner (messaggio verbatim: "bisogna sistemare alcune cose: ho ad esempio registrato come pagata la prestazione di Gualfredo Bruno di Clarafond, però c'era già un pagamento registrato a suo nome quindi è andato in credito di 90 euro, inoltre bisogna far si che in piani paziente la pagina scorra fino al di sopra del dock altrimenti non si possono aggiungere prestazioni, inoltre la pagina paziente ha questi tasti che portano ai vari sezioni che è un po troppo ingombrante bisogna trovare un modo").
...
- **3) Scheda paziente — tab troppo ingombranti**: una sessione precedente (POL-UI-017 R2) aveva deliberatamente spostato questa barra da uno scroller orizzontale a una griglia che va a capo (test di regressione esistente: mai `overflow-x`) — quindi la soluzione non poteva essere "tornare allo scroll orizzontale". `TABS` in `SchedaPaz.jsx` ora separa emoji e testo (`{id,emoji,label}` invece di `{id,l}`); su mobile il testo (`.patient-record-tabs__label`) si nasconde e restano solo le icone (title/aria-label conservano il nome completo per l'accessibilità), con `grid-template-columns:repeat(auto-fill,minmax(50px,1fr))` invece di 3 colonne fisse — fino a 4 righe di testo diventano 1-2 righe di icone, senza nascondere nessuna sezione e senza reintrodurre lo scroll. "Impianti" (che condivideva 🦷 con "Piani") ha ora la sua icona propria 🦴 per restare distinguibile in modalità solo-icona.
- VALIDATION: `npm test` 668/668 (5 nuovi/aggiornati: guardia di regressione sul residuo Incassato, banner già-pagato in IncassoModal, riserva di spazio sotto il dock, compattazione icon-only dei tab senza scroller); `npm run build` pulito; `git diff --check` pulito.
