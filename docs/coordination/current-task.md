...
- **Consigli per i prossimi giri (Fase 2, come richiesto)** — due controlli esplicitamente citati dal Product Owner NON sono ancora implementabili senza una piccola aggiunta di prodotto, spiegato anche in chat: (1) "panoramica caricata" per chi ha impianti — serve poter etichettare un documento come tipo "panoramica" in fase di caricamento (oggi nessuna UI lo distingue da un esame generico); (2) "bollette caricate corrette e non a caso" — controllo di qualità/anomalia sull'importo (fuori range storico ecc.), non solo presenza/recency come gli altri controlli spesa. Suggerisco anche di rendere i pesi dei controlli personalizzabili (oggi sono tutti pesati uguale nella media) se alcuni contano di più per lo studio (es. anamnesi/privacy più critici dell'anagrafica).
- VALIDATION: `npm test` 714/714 (nuovi: `tests/dataHealthScore.test.mjs` per il modulo puro, `tests/poliedronHealthScoreWidget.test.mjs` per il widget; aggiornati `tests/homeWidgetRegistry.test.mjs`/`tests/mobileHomeRound2.test.mjs` per il nuovo id widget); `npm run build` pulito; `git diff --check` pulito. Nessuna migration — solo componenti/selettori client, un fetch aggiuntivo di due sole colonne già coperte da RLS esistente.
