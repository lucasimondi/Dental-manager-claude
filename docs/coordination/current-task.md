...
- BRANCH: `feature/incassi-lettura-estratto-conto`, base `origin/master`.
- STATUS: IMPLEMENTED_NOT_MERGED — codice scritto, testato, edge function già deployata in produzione (legge soltanto, non scrive nulla lato server). **Nessun merge/PR-merge fatto**: in attesa di review/QA del Product Owner, non essendo stata data un'istruzione esplicita di merge per questo task (a differenza di POL-FIN-003).
...
- EXACT NEXT ACTION: Product Owner testa dal vivo "Leggi estratto conto" in Incassi con un vero estratto conto (immagine o PDF) e decide se aprire PR/mergiare. Non aperta nessuna PR finché non richiesto esplicitamente (diversamente da POL-FIN-003, qui non c'è stata un'istruzione di merge).
